# This file was generated by nvfetcher, please do not modify it manually.
{ fetchgit, fetchurl, fetchFromGitHub }:
{
  adguardhome-darwin-amd64 = {
    pname = "adguardhome-darwin-amd64";
    version = "0.108.0-b.15";
    src = fetchurl {
      url = "https://github.com/AdguardTeam/AdGuardHome/releases/download/v0.108.0-b.15/AdGuardHome_darwin_amd64.zip";
      sha256 = "sha256-qAGUX6iaZqg3flD+rQUNF/rFzIoRY2+DGg5abLhd6Ao=";
    };
  };
  adguardhome-linux-386 = {
    pname = "adguardhome-linux-386";
    version = "0.108.0-b.15";
    src = fetchurl {
      url = "https://github.com/AdguardTeam/AdGuardHome/releases/download/v0.108.0-b.15/AdGuardHome_linux_386.tar.gz";
      sha256 = "sha256-oyOu1RH8LD9BQ0VKj8RcbNNaMuSAS1ZKW0OWb2+5it4=";
    };
  };
  adguardhome-linux-amd64 = {
    pname = "adguardhome-linux-amd64";
    version = "0.108.0-b.15";
    src = fetchurl {
      url = "https://github.com/AdguardTeam/AdGuardHome/releases/download/v0.108.0-b.15/AdGuardHome_linux_amd64.tar.gz";
      sha256 = "sha256-/Y/hHkkIGo6YC9Siv1UfDsZh+Gp28dofqox94KZptDU=";
    };
  };
  adguardhome-linux-arm64 = {
    pname = "adguardhome-linux-arm64";
    version = "0.108.0-b.15";
    src = fetchurl {
      url = "https://github.com/AdguardTeam/AdGuardHome/releases/download/v0.108.0-b.15/AdGuardHome_linux_arm64.tar.gz";
      sha256 = "sha256-pbVeKbKzWNGRH+fYbVs7rCgvp0khfHR7fIs7avBPOAM=";
    };
  };
  adl = {
    pname = "adl";
    version = "65f68e1dcae4c0caa52668d3a854269e7d226f7c";
    src = fetchFromGitHub ({
      owner = "RaitaroH";
      repo = "adl";
      rev = "65f68e1dcae4c0caa52668d3a854269e7d226f7c";
      fetchSubmodules = false;
      sha256 = "sha256-huGpDtkWrhZyKDNKXat8T3qtAyMjBaq8HFd1w1ThUVk=";
    });
  };
  age-plugin-yubikey = {
    pname = "age-plugin-yubikey";
    version = "08ba0aa1ec3ced531dcb16a9db1795d6ee9b52cb";
    src = fetchFromGitHub ({
      owner = "str4d";
      repo = "age-plugin-yubikey";
      rev = "08ba0aa1ec3ced531dcb16a9db1795d6ee9b52cb";
      fetchSubmodules = false;
      sha256 = "sha256-KXqicTZ9GZlNj1AH3tMmOrC8zjXoEnqo4JJJTBdiI4E=";
    });
    cargoLock."Cargo.lock" = {
      lockFile = ./age-plugin-yubikey-08ba0aa1ec3ced531dcb16a9db1795d6ee9b52cb/Cargo.lock;
      outputHashes = {
        
      };
    };
  };
  anime-downloader = {
    pname = "anime-downloader";
    version = "24de83d4ef392e17f39710cc9054ff90e3602533";
    src = fetchFromGitHub ({
      owner = "anime-dl";
      repo = "anime-downloader";
      rev = "24de83d4ef392e17f39710cc9054ff90e3602533";
      fetchSubmodules = false;
      sha256 = "sha256-yS3lkmoqVNORG+H6x95mt68N8wh8X5/gji1fAl1RPqg=";
    });
  };
  any-nix-shell = {
    pname = "any-nix-shell";
    version = "67d8b02e605684c46a12445e689cda0c88fe278e";
    src = fetchFromGitHub ({
      owner = "haslersn";
      repo = "any-nix-shell";
      rev = "67d8b02e605684c46a12445e689cda0c88fe278e";
      fetchSubmodules = false;
      sha256 = "sha256-y9oOn1pDVPcY1ttWlTBZVptavgTwGQbADWpwe4yZG04=";
    });
  };
  arkenfox-userjs = {
    pname = "arkenfox-userjs";
    version = "3f09afdee07c5aea33f6dc4194e1425d6400a0d7";
    src = fetchFromGitHub ({
      owner = "arkenfox";
      repo = "user.js";
      rev = "3f09afdee07c5aea33f6dc4194e1425d6400a0d7";
      fetchSubmodules = false;
      sha256 = "sha256-NyP/kJlV1Ai7os3gMqukKMdRSQ78GAjPCJkZDZtDdeo=";
    });
  };
  asusctl = {
    pname = "asusctl";
    version = "af2462317828c638ee3c889669f771707fd58ba0";
    src = fetchgit {
      url = "https://gitlab.com/asus-linux/asusctl";
      rev = "af2462317828c638ee3c889669f771707fd58ba0";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sha256 = "sha256-VN7GeeVW53rNWieED6cJU3kwxKgXIIwWVuhHUrog9gc=";
    };
    cargoLock."Cargo.lock" = {
      lockFile = ./asusctl-af2462317828c638ee3c889669f771707fd58ba0/Cargo.lock;
      outputHashes = {
        "eframe-0.19.0" = "sha256-+5J4JmoVNir8GMb3Gz4xap9uvMQlvIJKJNsvxEQnRUk=";
      };
    };
  };
  avizo = {
    pname = "avizo";
    version = "67af5fd55279c25546d79cebdb4cb73440093082";
    src = fetchFromGitHub ({
      owner = "misterdanb";
      repo = "avizo";
      rev = "67af5fd55279c25546d79cebdb4cb73440093082";
      fetchSubmodules = false;
      sha256 = "sha256-bZE9uTAaKUFP3zw6pJB8xpZjN00vM7lntn5zG3+2CYc=";
    });
  };
  awesome = {
    pname = "awesome";
    version = "bde785ee64f91887cfe669f9db0dd8d9e90d32fc";
    src = fetchFromGitHub ({
      owner = "awesomeWM";
      repo = "awesome";
      rev = "bde785ee64f91887cfe669f9db0dd8d9e90d32fc";
      fetchSubmodules = false;
      sha256 = "sha256-aKXdZ/F7drzk5mePI+ViHcBvDzAmC8Xp/9Kro9reFbI=";
    });
  };
  awestore = {
    pname = "awestore";
    version = "5509786a1d5bc538dcdde027bd07b35bc29b0df1";
    src = fetchFromGitHub ({
      owner = "K4rakara";
      repo = "awestore";
      rev = "5509786a1d5bc538dcdde027bd07b35bc29b0df1";
      fetchSubmodules = false;
      sha256 = "sha256-VuMmEf5bCifXKHerTyFOYW4w4fXA83HM02Oj4VmQOeo=";
    });
  };
  betterdiscord-installer = {
    pname = "betterdiscord-installer";
    version = "1.1.1";
    src = fetchurl {
      url = "https://github.com/BetterDiscord/Installer/releases/download/v1.1.1/BetterDiscord-Linux.AppImage";
      sha256 = "sha256-EZj8vRyWbEfCagy/x3QTFADvE6Z76fqnDVRQZhD0S2k=";
    };
  };
  betterdiscordctl = {
    pname = "betterdiscordctl";
    version = "49a26fe82d441fbb8e51b0511969929f05cb2057";
    src = fetchFromGitHub ({
      owner = "bb010g";
      repo = "betterdiscordctl";
      rev = "49a26fe82d441fbb8e51b0511969929f05cb2057";
      fetchSubmodules = false;
      sha256 = "sha256-aBJDQAKmsBGoFaurADBTDqL9L/4+bWBk2y26vQ0LFx4=";
    });
  };
  bling = {
    pname = "bling";
    version = "70c894e58bb88dea55e9799ad373fdaea309da9e";
    src = fetchFromGitHub ({
      owner = "BlingCorp";
      repo = "bling";
      rev = "70c894e58bb88dea55e9799ad373fdaea309da9e";
      fetchSubmodules = false;
      sha256 = "sha256-sUUMgdAmXAB/2ocwoo2OTZlWRbSRLdKi1oZB20qRNWU=";
    });
  };
  bottles = {
    pname = "bottles";
    version = "05e7936d11b557011c9102353e0369c70d32e521";
    src = fetchFromGitHub ({
      owner = "bottlesdevs";
      repo = "Bottles";
      rev = "05e7936d11b557011c9102353e0369c70d32e521";
      fetchSubmodules = false;
      sha256 = "sha256-tnkqzckKZ34l/n7VAsmMx2lZjS0Mi2vOfAQVTIPerro=";
    });
  };
  caddy = {
    pname = "caddy";
    version = "v2.6.0-beta.3";
    src = fetchFromGitHub ({
      owner = "caddyserver";
      repo = "caddy";
      rev = "v2.6.0-beta.3";
      fetchSubmodules = false;
      sha256 = "sha256-PAm/XsxDwsnI7ICAz4867DzSNKurgL1/o4TcLyjaqzE=";
    });
  };
  caddy-dist = {
    pname = "caddy-dist";
    version = "v2.6.0-beta.3";
    src = fetchFromGitHub ({
      owner = "caddyserver";
      repo = "dist";
      rev = "v2.6.0-beta.3";
      fetchSubmodules = false;
      sha256 = "sha256-yw84ooXEqamWKANXmd5pU5Ig7ANDplBUwynF/qPLq1g=";
    });
  };
  caddy-git = {
    pname = "caddy-git";
    version = "258071d857aa8d1524161cb7273f960f24162ce9";
    src = fetchFromGitHub ({
      owner = "caddyserver";
      repo = "caddy";
      rev = "258071d857aa8d1524161cb7273f960f24162ce9";
      fetchSubmodules = false;
      sha256 = "sha256-POCwIehTuavmg2qY4aba7qMimxhu4wssQUEU3yhqiKw=";
    });
  };
  cinny = {
    pname = "cinny";
    version = "a8f374dd436ac58ac6101cc39ecabb453f844a4c";
    src = fetchFromGitHub ({
      owner = "cinnyapp";
      repo = "cinny";
      rev = "a8f374dd436ac58ac6101cc39ecabb453f844a4c";
      fetchSubmodules = false;
      sha256 = "sha256-7gcUWJcm19IA8+xyUPMo/dRYGYF6ir2c+zxBUEFm3UY=";
    });
  };
  cinny-bin = {
    pname = "cinny-bin";
    version = "2.1.3";
    src = fetchurl {
      url = "https://github.com/cinnyapp/cinny/releases/download/v2.1.3/cinny-v2.1.3.tar.gz";
      sha256 = "sha256-Tn8pT0tppBEQtLdwQf0edUBe3dkK7fedunnNYIld/SQ=";
    };
  };
  cinny-desktop = {
    pname = "cinny-desktop";
    version = "1d87374628e4a59c9f56a985419df39f89c78f97";
    src = fetchFromGitHub ({
      owner = "cinnyapp";
      repo = "cinny-desktop";
      rev = "1d87374628e4a59c9f56a985419df39f89c78f97";
      fetchSubmodules = true;
      sha256 = "sha256-Cvdj5R8J8HS10nAIn57GribLCUIANTs/z3hrZ1X69Iw=";
    });
    cargoLock."src-tauri/Cargo.lock" = {
      lockFile = ./cinny-desktop-1d87374628e4a59c9f56a985419df39f89c78f97/src-tauri/Cargo.lock;
      outputHashes = {
        
      };
    };
  };
  cinny-desktop-bin = {
    pname = "cinny-desktop-bin";
    version = "2.1.3";
    src = fetchurl {
      url = "https://github.com/cinnyapp/cinny-desktop/releases/download/v2.1.3/Cinny_desktop-x86_64.AppImage";
      sha256 = "sha256-lIiRhS3jwhjIpX4f3ofo5pEXpGdCMsPFroBNvXmyIxU=";
    };
  };
  conduit = {
    pname = "conduit";
    version = "147f27521c0d7dbc32d39ec1d8da6cd00008f23c";
    src = fetchgit {
      url = "https://gitlab.com/famedly/conduit";
      rev = "147f27521c0d7dbc32d39ec1d8da6cd00008f23c";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sha256 = "sha256-j469Zh8zyqJNWz7q6gjRu1Khk9y6Xbb52SpxzNjADW8=";
    };
    cargoLock."Cargo.lock" = {
      lockFile = ./conduit-147f27521c0d7dbc32d39ec1d8da6cd00008f23c/Cargo.lock;
      outputHashes = {
        "ruma-0.5.0" = "sha256-srmybII7j3xkSDVFvIiWUQnZsW+vEKKagSZhA5MRRQc=";
        "reqwest-0.11.9" = "sha256-wH/q7REnkz30ENBIK5Rlxnc1F6vOyuEANMHFmiVPaGw=";
        "heed-0.10.6" = "sha256-rm02pJ6wGYN4SsAbp85jBVHDQ5ITjZZd+79EC2ubRsY=";
      };
    };
  };
  conduit-toolbox = {
    pname = "conduit-toolbox";
    version = "805baa070528e6db7161ef35b75180e70d896541";
    src = fetchFromGitHub ({
      owner = "ShadowJonathan";
      repo = "conduit_toolbox";
      rev = "805baa070528e6db7161ef35b75180e70d896541";
      fetchSubmodules = false;
      sha256 = "sha256-vQ4r2Cvf2okeJ5qGIQ8mHoUmmrbUbY+bIEpee7XH6zA=";
    });
    cargoLock."Cargo.lock" = {
      lockFile = ./conduit-toolbox-805baa070528e6db7161ef35b75180e70d896541/Cargo.lock;
      outputHashes = {
        "heed-0.10.6" = "sha256-rm02pJ6wGYN4SsAbp85jBVHDQ5ITjZZd+79EC2ubRsY=";
      };
    };
  };
  cups-pdf = {
    pname = "cups-pdf";
    version = "3.0.1";
    src = fetchurl {
      url = "https://www.cups-pdf.de/src/cups-pdf_3.0.1.tar.gz";
      sha256 = "sha256-c4Zp7f9/FGn+XkESAth/k7oltF8zKmI/tgfUnFmqlTE=";
    };
  };
  discord-canary = {
    pname = "discord-canary";
    version = "0.0.139";
    src = fetchurl {
      url = "https://dl-canary.discordapp.net/apps/linux/0.0.139/discord-canary-0.0.139.tar.gz";
      sha256 = "sha256-/PfO0TWRxMrK+V1XkYmdaXQ6SfyJNBFETaR9oV90itI=";
    };
  };
  dnscrypt-proxy2 = {
    pname = "dnscrypt-proxy2";
    version = "ca253923eb631ccde49bfcc6cf376ba867485108";
    src = fetchFromGitHub ({
      owner = "DNSCrypt";
      repo = "dnscrypt-proxy";
      rev = "ca253923eb631ccde49bfcc6cf376ba867485108";
      fetchSubmodules = false;
      sha256 = "sha256-1Mju3OguRm/ATQovS0P2Xqgj/H3UP6Lb852PKzuYtu8=";
    });
  };
  doggo = {
    pname = "doggo";
    version = "0da9178683b435abf80d5e2a229cd8d30a0f64fc";
    src = fetchFromGitHub ({
      owner = "mr-karan";
      repo = "doggo";
      rev = "0da9178683b435abf80d5e2a229cd8d30a0f64fc";
      fetchSubmodules = false;
      sha256 = "sha256-LGxu4A8s12/OgAMZ5Ua8mbXbnjk9WbzlDk4AtRbr//4=";
    });
  };
  dribbblish-dynamic-theme = {
    pname = "dribbblish-dynamic-theme";
    version = "8da36b8e5f07a20af2ca8a1dd3c81eedf7282fa1";
    src = fetchFromGitHub ({
      owner = "JulienMaille";
      repo = "dribbblish-dynamic-theme";
      rev = "8da36b8e5f07a20af2ca8a1dd3c81eedf7282fa1";
      fetchSubmodules = false;
      sha256 = "sha256-70wtFxJOY9h4moT4hCTs4yfp2wGM39i5qnJerBVkpV4=";
    });
  };
  droidcam = {
    pname = "droidcam";
    version = "d1c3206306f948a8e86c7a256f1ca1be09b114b1";
    src = fetchFromGitHub ({
      owner = "aramg";
      repo = "droidcam";
      rev = "d1c3206306f948a8e86c7a256f1ca1be09b114b1";
      fetchSubmodules = false;
      sha256 = "sha256-a6pB4WgRWbJCseRWlMFLrTURFX6c/lfsuBfIiTmaDPA=";
    });
  };
  duf = {
    pname = "duf";
    version = "943f84cf435b9dfdb894edb3729e115181525302";
    src = fetchFromGitHub ({
      owner = "muesli";
      repo = "duf";
      rev = "943f84cf435b9dfdb894edb3729e115181525302";
      fetchSubmodules = false;
      sha256 = "sha256-rvVMFhnPfTGdjz1a0Io+GnutEw1aEy/NXFNRECNvpQQ=";
    });
  };
  eww = {
    pname = "eww";
    version = "52e9603dd8c79cbc267a4733389c1f7e7625b352";
    src = fetchFromGitHub ({
      owner = "elkowar";
      repo = "eww";
      rev = "52e9603dd8c79cbc267a4733389c1f7e7625b352";
      fetchSubmodules = false;
      sha256 = "sha256-zMJslerxTcSQ+A8fI4gFcJioRGs+T55343zOtsnEJ3k=";
    });
    cargoLock."Cargo.lock" = {
      lockFile = ./eww-52e9603dd8c79cbc267a4733389c1f7e7625b352/Cargo.lock;
      outputHashes = {
        
      };
    };
  };
  fake-background-webcam = {
    pname = "fake-background-webcam";
    version = "d222c2c7e9a8dad20cc5960caa88bef70e4c23ca";
    src = fetchFromGitHub ({
      owner = "fangfufu";
      repo = "Linux-Fake-Background-Webcam";
      rev = "d222c2c7e9a8dad20cc5960caa88bef70e4c23ca";
      fetchSubmodules = false;
      sha256 = "sha256-sxw93NQznc6QmHHN4eStIYzTYb9yremVOCNcb+Ut3gQ=";
    });
  };
  firefox-sidebar = {
    pname = "firefox-sidebar";
    version = "586c9ea4612b302ef85081e35b7368d2f7d5defd";
    src = fetchFromGitHub ({
      owner = "drannex42";
      repo = "FirefoxSidebar";
      rev = "586c9ea4612b302ef85081e35b7368d2f7d5defd";
      fetchSubmodules = false;
      sha256 = "sha256-wExL88eRr+5wTQ6NP08f1mPq9/+2Xu1SIsJv1WdTVuU=";
    });
  };
  flameshot = {
    pname = "flameshot";
    version = "001726a565049ebfeb237a3a39b77104899b970e";
    src = fetchFromGitHub ({
      owner = "flameshot-org";
      repo = "flameshot";
      rev = "001726a565049ebfeb237a3a39b77104899b970e";
      fetchSubmodules = false;
      sha256 = "sha256-lHVxvC/kxLLg9Ev9bpFGdiDLaFrT08Um44fWTmEIiDQ=";
    });
  };
  fluffychat = {
    pname = "fluffychat";
    version = "a9a93747180020453f1e0d79e1c737f400d56851";
    src = fetchgit {
      url = "https://gitlab.com/famedly/fluffychat";
      rev = "a9a93747180020453f1e0d79e1c737f400d56851";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sha256 = "sha256-32Q/nParMRrpnvBtFlpCPIvL8RK3W3sxLUk/SySHdU4=";
    };
  };
  flyingfox = {
    pname = "flyingfox";
    version = "4a6ba46cafc189ad628ee13bb8d3eef3e69fbced";
    src = fetchFromGitHub ({
      owner = "akshat46";
      repo = "FlyingFox";
      rev = "4a6ba46cafc189ad628ee13bb8d3eef3e69fbced";
      fetchSubmodules = false;
      sha256 = "sha256-ZUTQ2VvIAEPbLnEX2AYScAGTM3hWp7vUuGCz2wd8qG0=";
    });
  };
  frece = {
    pname = "frece";
    version = "d4071c6c852e68dda82b7ab0bfb0f141b39d4484";
    src = fetchFromGitHub ({
      owner = "YodaEmbedding";
      repo = "frece";
      rev = "d4071c6c852e68dda82b7ab0bfb0f141b39d4484";
      fetchSubmodules = false;
      sha256 = "sha256-lLSRLcnUJqWCEIJEOVHANzxTI+xR3VegRHtjHHI0gTs=";
    });
    cargoLock."Cargo.lock" = {
      lockFile = ./frece-d4071c6c852e68dda82b7ab0bfb0f141b39d4484/Cargo.lock;
      outputHashes = {
        
      };
    };
  };
  fx_cast = {
    pname = "fx_cast";
    version = "cc7641da030dd39469c3cce544872ce5919edfa9";
    src = fetchFromGitHub ({
      owner = "hensm";
      repo = "fx_cast";
      rev = "cc7641da030dd39469c3cce544872ce5919edfa9";
      fetchSubmodules = false;
      sha256 = "sha256-Aqu6u7nypJG7Mpzz3Ke/cfXyl7E09lopAKlk+ZFh344=";
    });
  };
  godu = {
    pname = "godu";
    version = "56dff7ee254e3770a2f9bd100f78671dc16862d6";
    src = fetchFromGitHub ({
      owner = "viktomas";
      repo = "godu";
      rev = "56dff7ee254e3770a2f9bd100f78671dc16862d6";
      fetchSubmodules = false;
      sha256 = "sha256-P6X/LiU3XPm25BlvQ6ENe/MFoNhGzfUvAUcgPA8+fQ4=";
    });
  };
  guiscrcpy = {
    pname = "guiscrcpy";
    version = "bb2bf8e2cf998fc4403b995fd993b175663dfec3";
    src = fetchFromGitHub ({
      owner = "srevinsaju";
      repo = "guiscrcpy";
      rev = "bb2bf8e2cf998fc4403b995fd993b175663dfec3";
      fetchSubmodules = false;
      sha256 = "sha256-c6Tg8G5w3Hu3K6rB08cDCz+1CkzVxb+tVpoNqAyOPQ0=";
    });
  };
  hyprland = {
    pname = "hyprland";
    version = "037d4ed422bb3bb34617f444a360a8abab6863cb";
    src = fetchFromGitHub ({
      owner = "hyprwm";
      repo = "hyprland";
      rev = "037d4ed422bb3bb34617f444a360a8abab6863cb";
      fetchSubmodules = false;
      sha256 = "sha256-BGN7XiiLx1D03L5j6OL7WPknxlt5cSejqfR+xQXpv5E=";
    });
  };
  ibus = {
    pname = "ibus";
    version = "babad7839ba6d72609c717d647bb2928724b4cc3";
    src = fetchFromGitHub ({
      owner = "ibus";
      repo = "ibus";
      rev = "babad7839ba6d72609c717d647bb2928724b4cc3";
      fetchSubmodules = false;
      sha256 = "sha256-tToZDNimJeMWGopKK8IW/OYQ/VAYycW9FXChvgK/YM4=";
    });
  };
  ibus-bamboo = {
    pname = "ibus-bamboo";
    version = "3380c09d5812cec18ff7ed80e7e6d69d6afd3ab2";
    src = fetchFromGitHub ({
      owner = "BambooEngine";
      repo = "ibus-bamboo";
      rev = "3380c09d5812cec18ff7ed80e7e6d69d6afd3ab2";
      fetchSubmodules = false;
      sha256 = "sha256-IkmNNYyV3eRAqTdh19d1hgNWtENmlMD5qYAGs34T3ZU=";
    });
  };
  inih = {
    pname = "inih";
    version = "r56";
    src = fetchFromGitHub ({
      owner = "benhoyt";
      repo = "inih";
      rev = "r56";
      fetchSubmodules = false;
      sha256 = "sha256-7k3i3pElihastUDrdf9DyRZMe2UNFckfLUFGb4rbWLo=";
    });
  };
  jibri = {
    pname = "jibri";
    version = "8.0-137-g260cee3";
    src = fetchurl {
      url = "https://download.jitsi.org/unstable/jibri_8.0-137-g260cee3-1_all.deb";
      sha256 = "sha256-jfp8rwVFcbCuVNW/yWcqxLXcn9YG26SYwwfh0JjGgdw=";
    };
  };
  jicofo = {
    pname = "jicofo";
    version = "1.0-922";
    src = fetchurl {
      url = "https://download.jitsi.org/unstable/jicofo_1.0-922-1_all.deb";
      sha256 = "sha256-USXIAnpcAerEnvPPFGhBDSwWC+Apgv0Jjio0ejVMcC8=";
    };
  };
  jitsi-meet = {
    pname = "jitsi-meet";
    version = "1.0.6570";
    src = fetchurl {
      url = "https://download.jitsi.org/jitsi-meet/src/jitsi-meet-1.0.6570.tar.bz2";
      sha256 = "sha256-eSqgwTXzyg1gA5p74lOJHTECXlUEXb6uziZUZBpmGHk=";
    };
  };
  jitsi-meet-prosody = {
    pname = "jitsi-meet-prosody";
    version = "1.0.6570";
    src = fetchurl {
      url = "https://download.jitsi.org/unstable/jitsi-meet-prosody_1.0.6570-1_all.deb";
      sha256 = "sha256-xZ5/WTM+qLdHgFW3KKdPL8xLYzEsw2giPcY51Se4gfQ=";
    };
  };
  jitsi-videobridge2 = {
    pname = "jitsi-videobridge2";
    version = "2.2-43-gce94dbb2";
    src = fetchurl {
      url = "https://download.jitsi.org/unstable/jitsi-videobridge2_2.2-43-gce94dbb2-1_all.deb";
      sha256 = "sha256-RT0HKMhCp/kMs0cJq7594lskGcCVSQe/H6u+RiLinEA=";
    };
  };
  layout-machi = {
    pname = "layout-machi";
    version = "d15b877fb5b0a5f90f010871c7dbfb163b874fe4";
    src = fetchFromGitHub ({
      owner = "xinhaoyuan";
      repo = "layout-machi";
      rev = "d15b877fb5b0a5f90f010871c7dbfb163b874fe4";
      fetchSubmodules = false;
      sha256 = "sha256-3oC2DOqxp/Kpcrt2g+KxpfQkh9XSrTVSHZ7yxQblNeI=";
    });
  };
  leonflix = {
    pname = "leonflix";
    version = "0.7.0";
    src = fetchurl {
      url = "https://leonflix.net/downloads/Leonflix.AppImage";
      sha256 = "sha256-Fok8OPP9dWjFyIVWUud+UDLiG/lUBNisuCcI0ipjHi4=";
    };
  };
  light = {
    pname = "light";
    version = "c5fb45423ec1ee1080e442e061ccfcdb98b32e4a";
    src = fetchFromGitHub ({
      owner = "haikarainen";
      repo = "light";
      rev = "c5fb45423ec1ee1080e442e061ccfcdb98b32e4a";
      fetchSubmodules = false;
      sha256 = "sha256-8v9ytywv2+/qqN94g44vHMGsbKueYSDXq76wo95pSFE=";
    });
  };
  lua-pam = {
    pname = "lua-pam";
    version = "482071137257e55dac62a510f792104a9d910ea1";
    src = fetchFromGitHub ({
      owner = "RMTT";
      repo = "lua-pam";
      rev = "482071137257e55dac62a510f792104a9d910ea1";
      fetchSubmodules = false;
      sha256 = "sha256-Kn0ozckmgxs19TmCuSChdiYMch92eif7DTEK+UoBtjw=";
    });
  };
  manix = {
    pname = "manix";
    version = "d08e7ca185445b929f097f8bfb1243a8ef3e10e4";
    src = fetchFromGitHub ({
      owner = "mlvzk";
      repo = "manix";
      rev = "d08e7ca185445b929f097f8bfb1243a8ef3e10e4";
      fetchSubmodules = false;
      sha256 = "sha256-GqPuYscLhkR5E2HnSFV4R48hCWvtM3C++3zlJhiK/aw=";
    });
  };
  mapcidr = {
    pname = "mapcidr";
    version = "v1.0.2";
    src = fetchFromGitHub ({
      owner = "projectdiscovery";
      repo = "mapcidr";
      rev = "v1.0.2";
      fetchSubmodules = false;
      sha256 = "sha256-qfEm+DFnTmGwHV7Sb488qFmjT+7QSCmbT4n5JhtDo5g=";
    });
  };
  matrix-appservice-discord = {
    pname = "matrix-appservice-discord";
    version = "ac5180ed11e67ea9c1e6467941148b90dd122fb5";
    src = fetchFromGitHub ({
      owner = "matrix-org";
      repo = "matrix-appservice-discord";
      rev = "ac5180ed11e67ea9c1e6467941148b90dd122fb5";
      fetchSubmodules = false;
      sha256 = "sha256-U+/eFyrXQSHkY70+WvGsqCTFHIArhbolT+W93uBE3EA=";
    });
  };
  microsoft-edge-beta = {
    pname = "microsoft-edge-beta";
    version = "106.0.1370.15";
    src = fetchurl {
      url = "https://packages.microsoft.com/repos/edge/pool/main/m/microsoft-edge-beta/microsoft-edge-beta_106.0.1370.15-1_amd64.deb";
      sha256 = "sha256-naXM5EGo7IFH0SH23LGBoWYIwvR3YD3886izFyOeHsM=";
    };
  };
  microsoft-edge-dev = {
    pname = "microsoft-edge-dev";
    version = "107.0.1375.0";
    src = fetchurl {
      url = "https://packages.microsoft.com/repos/edge/pool/main/m/microsoft-edge-dev/microsoft-edge-dev_107.0.1375.0-1_amd64.deb";
      sha256 = "sha256-JDbX8kBU80VZnQvAXNjKaWA3UQ2uCi1yC8GVmb0kAag=";
    };
  };
  microsoft-edge-stable = {
    pname = "microsoft-edge-stable";
    version = "105.0.1343.42";
    src = fetchurl {
      url = "https://packages.microsoft.com/repos/edge/pool/main/m/microsoft-edge-stable/microsoft-edge-stable_105.0.1343.42-1_amd64.deb";
      sha256 = "sha256-zVTtn8TVzwNFd9P2LmeeAEhQI+XDCokS7QmlYnDAVqI=";
    };
  };
  minecraft-wayland = {
    pname = "minecraft-wayland";
    version = "370ce5b95e3ae9bc4618fb45113bc641fbb13867";
    src = fetchFromGitHub ({
      owner = "Admicos";
      repo = "minecraft-wayland";
      rev = "370ce5b95e3ae9bc4618fb45113bc641fbb13867";
      fetchSubmodules = false;
      sha256 = "sha256-RPRg6Gd7N8yyb305V607NTC1kUzvyKiWsh6QlfHW+JE=";
    });
  };
  netdata = {
    pname = "netdata";
    version = "1.36.1";
    src = fetchurl {
      url = "https://github.com/netdata/netdata/archive/refs/tags/v1.36.1.tar.gz";
      sha256 = "sha256-mDSBDmSqANx5ELFf60kM8vPkx4fCGN6j0yrPGDRHnSg=";
    };
  };
  netdata-go-d-plugin = {
    pname = "netdata-go-d-plugin";
    version = "0.39.0";
    src = fetchurl {
      url = "https://github.com/netdata/go.d.plugin/archive/refs/tags/v0.39.0.tar.gz";
      sha256 = "sha256-aJ65bk0UUcnbxS5I9BsgQ8CwHk8x188MTWomvr2P4HE=";
    };
  };
  nix-zsh-completions = {
    pname = "nix-zsh-completions";
    version = "468d8cf752a62b877eba1a196fbbebb4ce4ebb6f";
    src = fetchFromGitHub ({
      owner = "spwhitt";
      repo = "nix-zsh-completions";
      rev = "468d8cf752a62b877eba1a196fbbebb4ce4ebb6f";
      fetchSubmodules = false;
      sha256 = "sha256-TWgo56l+FBXssOYWlAfJ5j4pOHNmontOEolcGdihIJs=";
    });
  };
  ntfs2btrfs = {
    pname = "ntfs2btrfs";
    version = "d92502d8d25bdd25d0c0c203880432c43f49b3ab";
    src = fetchFromGitHub ({
      owner = "maharmstone";
      repo = "ntfs2btrfs";
      rev = "d92502d8d25bdd25d0c0c203880432c43f49b3ab";
      fetchSubmodules = false;
      sha256 = "sha256-b7J27+7fhLcSoyO3D3NhTaUIr0uRiOM3fOotrq3KfNc=";
    });
  };
  nwg-drawer = {
    pname = "nwg-drawer";
    version = "583ddfde0bd3ebc93e03f047d6419f696826803e";
    src = fetchFromGitHub ({
      owner = "nwg-piotr";
      repo = "nwg-drawer";
      rev = "583ddfde0bd3ebc93e03f047d6419f696826803e";
      fetchSubmodules = false;
      sha256 = "sha256-eC9wLJvd3iXd2oeY+0lShnEfKYkgi4QT4D7fSG6NzE8=";
    });
  };
  ny-font = {
    pname = "ny-font";
    version = "0.0.0.20210625";
    src = fetchurl {
      url = "https://devimages-cdn.apple.com/design/resources/download/NY.dmg";
      sha256 = "sha256-Rr0UpJa7kemczCqNn6b8HNtW6PiWO/Ez1LUh/WNk8S8=";
    };
  };
  ouch = {
    pname = "ouch";
    version = "ff8619acb7c5ea92b17674421e7dc422b8ccbeb0";
    src = fetchFromGitHub ({
      owner = "ouch-org";
      repo = "ouch";
      rev = "ff8619acb7c5ea92b17674421e7dc422b8ccbeb0";
      fetchSubmodules = false;
      sha256 = "sha256-Av1+ib41XRsWoH/nMpzyfwetnKtswtXlAC1QXCbkweY=";
    });
    cargoLock."Cargo.lock" = {
      lockFile = ./ouch-ff8619acb7c5ea92b17674421e7dc422b8ccbeb0/Cargo.lock;
      outputHashes = {
        
      };
    };
  };
  paper = {
    pname = "paper";
    version = "35ecb75c6776615f1db5dfb861fbe407263d8057";
    src = fetchgit {
      url = "https://gitlab.com/snakedye/paper";
      rev = "35ecb75c6776615f1db5dfb861fbe407263d8057";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sha256 = "sha256-D0VMUZ7QN4xTcTBulzSlt7IvjRxPYFiTs1AcuhNKrek=";
    };
    cargoLock."Cargo.lock" = {
      lockFile = ./paper-35ecb75c6776615f1db5dfb861fbe407263d8057/Cargo.lock;
      outputHashes = {
        "snui-0.1.0" = "sha256-y61EWKr5qD9OFrMVQs9SVyLCqq5lOwLXCAo2w0AYt7M=";
      };
    };
  };
  picom = {
    pname = "picom";
    version = "f2970bc697bdf20d398d1be05ff72d50df911e64";
    src = fetchFromGitHub ({
      owner = "yshui";
      repo = "picom";
      rev = "f2970bc697bdf20d398d1be05ff72d50df911e64";
      fetchSubmodules = false;
      sha256 = "sha256-aeAXMKwjQv/kRd7lTeIrcJmqT1vAUkRUOM8VrEJpd8M=";
    });
  };
  playit-agent = {
    pname = "playit-agent";
    version = "e986524ba9ac51e821a0bb3e363dcd1acd90b314";
    src = fetchFromGitHub ({
      owner = "playit-cloud";
      repo = "playit-agent";
      rev = "e986524ba9ac51e821a0bb3e363dcd1acd90b314";
      fetchSubmodules = false;
      sha256 = "sha256-Y7CFcPJ/lKYZ6VwKO6p/oWYw4YasjMecfRraiygcRSI=";
    });
    cargoLock."Cargo.lock" = {
      lockFile = ./playit-agent-e986524ba9ac51e821a0bb3e363dcd1acd90b314/Cargo.lock;
      outputHashes = {
        "webbrowser-0.5.5" = "sha256-9LhheWDmQKUdKLsYo5Uo+Z3PlKeMMnZvfWyXOMvEf7M=";
      };
    };
  };
  plymouth-themes = {
    pname = "plymouth-themes";
    version = "bf2f570bee8e84c5c20caac353cbe1d811a4745f";
    src = fetchFromGitHub ({
      owner = "adi1090x";
      repo = "plymouth-themes";
      rev = "bf2f570bee8e84c5c20caac353cbe1d811a4745f";
      fetchSubmodules = false;
      sha256 = "sha256-VNGvA8ujwjpC2rTVZKrXni2GjfiZk7AgAn4ZB4Baj2k=";
    });
  };
  pmbootstrap = {
    pname = "pmbootstrap";
    version = "1.46.0";
    src = fetchurl {
      url = "https://pypi.io/packages/source/p/pmbootstrap/pmbootstrap-1.46.0.tar.gz";
      sha256 = "sha256-azr2B8ng8aJJlv4tqozup20iWeogqbZbzdMXS/9PoQQ=";
    };
  };
  pmbootstrap-test = {
    pname = "pmbootstrap-test";
    version = "22e0f64095fbe42a0f087858dc4a6cf02387b0d8";
    src = fetchgit {
      url = "https://gitlab.com/postmarketOS/pmbootstrap";
      rev = "22e0f64095fbe42a0f087858dc4a6cf02387b0d8";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sha256 = "sha256-mcOCk68FXU1I+/cmsyacay+Q/iTw3z+ytxzsffDads4=";
    };
  };
  pure = {
    pname = "pure";
    version = "47c0c881f0e7cfdb5eaccd335f52ad17b897c060";
    src = fetchFromGitHub ({
      owner = "sindresorhus";
      repo = "pure";
      rev = "47c0c881f0e7cfdb5eaccd335f52ad17b897c060";
      fetchSubmodules = false;
      sha256 = "sha256-e1D+9EejlVZxOyErg6eRgawth5gAhv6KpgjhK06ErZc=";
    });
  };
  pythonPackages-cfscrape = {
    pname = "pythonPackages-cfscrape";
    version = "e510962c608382bcef5de75033d60cc98cb9561d";
    src = fetchurl {
      url = "https://github.com/Anorov/cloudflare-scrape/archive/master.tar.gz";
      sha256 = "sha256-+pFN1Ts23Ggm6YZueyLdQaMJCfox/CRmMelzGvWRtEw=";
    };
  };
  pythonPackages-cmapy = {
    pname = "pythonPackages-cmapy";
    version = "0.6.6";
    src = fetchurl {
      url = "https://pypi.io/packages/source/c/cmapy/cmapy-0.6.6.tar.gz";
      sha256 = "sha256-y1KmswV8SaFG+wlkuDAvL7fWHf5q5t4amLY2qs6AUlU=";
    };
  };
  pythonPackages-mediapipe = {
    pname = "pythonPackages-mediapipe";
    version = "0.8.8";
    src = fetchurl {
      url = "https://files.pythonhosted.org/packages/ed/5f/188b509ab93e740f3fd8d9179de0ef9340852b48f1f5d671cd090a54996f/mediapipe-0.8.8-cp39-cp39-manylinux_2_17_x86_64.manylinux2014_x86_64.whl";
      sha256 = "sha256-qW1D06zrH8HnZiQoO5PC+8mK8z1HcZJxU9hmwyi8FZc=";
    };
  };
  pythonPackages-pySmartDL = {
    pname = "pythonPackages-pySmartDL";
    version = "b93df794e1e60017c42d9520ac097b6fd38c2e8b";
    src = fetchurl {
      url = "https://github.com/iTaybb/pySmartDL/archive/master.tar.gz";
      sha256 = "sha256-26TGmz9eIXm6epKUnvNDQymRBOC3TcpxCXFbhuVfVto=";
    };
  };
  pywalfox = {
    pname = "pywalfox";
    version = "2.7.4";
    src = fetchurl {
      url = "https://pypi.io/packages/source/p/pywalfox/pywalfox-2.7.4.tar.gz";
      sha256 = "sha256-Wec9fic4lXT7gBY04D2EcfCb/gYoZcrYA/aMRWaA7WY=";
    };
  };
  quibble = {
    pname = "quibble";
    version = "f98986d884bd825572111f58e905d2e1bb1b7ba0";
    src = fetchFromGitHub ({
      owner = "maharmstone";
      repo = "quibble";
      rev = "f98986d884bd825572111f58e905d2e1bb1b7ba0";
      fetchSubmodules = false;
      sha256 = "sha256-lzqc6fB0YkxwNAZQeNMCcpL/wzI59PC57CmnYPQO6qI=";
    });
  };
  rainfox = {
    pname = "rainfox";
    version = "231a8434a3c1aedd64dd8ca4cf2e67318440c9ff";
    src = fetchFromGitHub ({
      owner = "1280px";
      repo = "rainfox";
      rev = "231a8434a3c1aedd64dd8ca4cf2e67318440c9ff";
      fetchSubmodules = false;
      sha256 = "sha256-muDz1rFTvWOZGrEzql+VZbs42bS09BREjKS5OpBu2g8=";
    });
  };
  redshift = {
    pname = "redshift";
    version = "7da875d34854a6a34612d5ce4bd8718c32bec804";
    src = fetchFromGitHub ({
      owner = "minus7";
      repo = "redshift";
      rev = "7da875d34854a6a34612d5ce4bd8718c32bec804";
      fetchSubmodules = false;
      sha256 = "sha256-pyR7xNdi/83NSoC2WIrX8U+A6nU+vZBlePLXrQZnc1k=";
    });
  };
  retroarch = {
    pname = "retroarch";
    version = "83ce6f42d9720f9ce3dbfeca21a73322f49a20c8";
    src = fetchFromGitHub ({
      owner = "libretro";
      repo = "retroarch";
      rev = "83ce6f42d9720f9ce3dbfeca21a73322f49a20c8";
      fetchSubmodules = false;
      sha256 = "sha256-DulHybX2eLX0W/lw6HfT5zkmlLvH9u+vpqm2e/CPbnU=";
    });
  };
  revanced-cli = {
    pname = "revanced-cli";
    version = "2.10.1";
    src = fetchurl {
      url = "https://github.com/revanced/revanced-cli/releases/download/v2.10.1/revanced-cli-2.10.1-all.jar";
      sha256 = "sha256-iG6PUvM2q96TT6AgzX7drNLsA6dc5cWRNinn3sPJPSA=";
    };
  };
  revanced-integrations = {
    pname = "revanced-integrations";
    version = "0.37.3";
    src = fetchurl {
      url = "https://github.com/revanced/revanced-integrations/releases/download/v0.37.3/app-release-unsigned.apk";
      sha256 = "sha256-UEncOcx7ECHtYcnzm6fCWODIyyUIBpVKarmwpEE4XvE=";
    };
  };
  revanced-patches-jar = {
    pname = "revanced-patches-jar";
    version = "2.57.0";
    src = fetchurl {
      url = "https://github.com/revanced/revanced-patches/releases/download/v2.57.0/revanced-patches-2.57.0.jar";
      sha256 = "sha256-ZVpcaWHW5HIcaBaeW7ia+6nlbJQFBbMFNVA9BQwqd8Q=";
    };
  };
  rofi-wayland = {
    pname = "rofi-wayland";
    version = "8995b575df086a4033332a25ced3afc7925c3607";
    src = fetchFromGitHub ({
      owner = "lbonn";
      repo = "rofi";
      rev = "8995b575df086a4033332a25ced3afc7925c3607";
      fetchSubmodules = true;
      sha256 = "sha256-3+9gaql2F6U6iGWxGAmx30qjWlVdMs0IdIbkC6Fs7no=";
    });
  };
  rustdesk = {
    pname = "rustdesk";
    version = "9bb52db1cd92f797c2764e187784e5ecdec5e8c7";
    src = fetchFromGitHub ({
      owner = "rustdesk";
      repo = "rustdesk";
      rev = "9bb52db1cd92f797c2764e187784e5ecdec5e8c7";
      fetchSubmodules = false;
      sha256 = "sha256-ZDnbg+D2GiNIiYJwtiKiALriejzyo9cF8k7eVdbiG64=";
    });
    cargoLock."Cargo.lock" = {
      lockFile = ./rustdesk-9bb52db1cd92f797c2764e187784e5ecdec5e8c7/Cargo.lock;
      outputHashes = {
        "trayicon-0.1.3-1" = "sha256-shLsOncL3ySRN/m9SjvjurneMaXhGDWks0QF3pVz6J0=";
        "tfc-0.6.1" = "sha256-3CYvFmM0oXyJcX0/ceOIbNofDMbUlGD2Rzjhghb7o4g=";
        "rust-pulsectl-0.2.12" = "sha256-8jXTspWvjONFcvw9/Z8C43g4BuGZ3rsG32tvLMQbtbM=";
        "confy-0.4.0" = "sha256-e91cvEixhpPzIthAxzTa3fDY6eCsHUy/eZQAqs7QTDo=";
        "x11-2.19.0" = "sha256-GDCeKzUtvaLeBDmPQdyr499EjEfT6y4diBMzZVEptzc=";
        "parity-tokio-ipc-0.7.3-1" = "sha256-eULJePtBu0iBI3It/bPH0h82Obsb1PJALgwYwrnCFYI=";
        "tokio-socks-0.5.1-1" = "sha256-45QQ6FrhGU9uEhbKXTKd/mY6MDumO6p46NmlakdyDQk=";
        "hwcodec-0.1.0" = "sha256-VddI5p/Hfa8VhjjhjA4KDDkThFmi8JcH6oeaGG6/dtA=";
        "mouce-0.2.1" = "sha256-YcS7rn3eWmFWIq3X6ytwPRp3eWuXjJwseqsMoA6k7/w=";
        "rdev-0.5.0-2" = "sha256-gkZN4k2id/DxFu8JOKoHlIgbk8EsV937t8GJy37qKUs=";
        "sciter-rs-0.5.57" = "sha256-ZZnZDhMjK0LjgmK0da1yvB0uoKueLhhhQtzmjoN+1R0=";
        "magnum-opus-0.4.0" = "sha256-jNscS3DNJuoZE0+s4RFDCIuL/xo9uUzBiDvTcXKcoNM=";
        "evdev-0.11.5" = "sha256-aoPmjGi/PftnH6ClEWXHvIj0X3oh15ZC1q7wPC1XPr0=";
        "flutter_rust_bridge-1.32.0" = "sha256-LE5KHzP/zy4zeGiyFx+kDitP3LC2tiPGN5mp0/urPQs=";
      };
    };
  };
  rustdesk-server = {
    pname = "rustdesk-server";
    version = "74cb82c8a205fc459750b84e1ef3678d5d905316";
    src = fetchFromGitHub ({
      owner = "rustdesk";
      repo = "rustdesk-server";
      rev = "74cb82c8a205fc459750b84e1ef3678d5d905316";
      fetchSubmodules = false;
      sha256 = "sha256-w9cy4pdAeYB7i9EOuLYoDnM/oh6C9PpSYOoifVdWhjc=";
    });
    cargoLock."Cargo.lock" = {
      lockFile = ./rustdesk-server-74cb82c8a205fc459750b84e1ef3678d5d905316/Cargo.lock;
      outputHashes = {
        "confy-0.4.0" = "sha256-e91cvEixhpPzIthAxzTa3fDY6eCsHUy/eZQAqs7QTDo=";
        "async-speed-limit-0.3.1" = "sha256-iOel6XA07RPrBjQAFLnxXX4VBpDrYZaqQc9clnsOorI=";
        "tokio-socks-0.5.1" = "sha256-inmAJk0fAlsVNIwfD/M+htwIdQHwGSTRrEy6N/mspMI=";
      };
    };
  };
  sddm = {
    pname = "sddm";
    version = "02801666f7c9e82b57fe8198e2084d8fdcff6b91";
    src = fetchFromGitHub ({
      owner = "sddm";
      repo = "sddm";
      rev = "02801666f7c9e82b57fe8198e2084d8fdcff6b91";
      fetchSubmodules = false;
      sha256 = "sha256-KNj1l4jW7moFOC1JkgRqQR0uPcSCMUuVnyKmkL0sDZw=";
    });
  };
  sddm-chili = {
    pname = "sddm-chili";
    version = "6516d50176c3b34df29003726ef9708813d06271";
    src = fetchFromGitHub ({
      owner = "MarianArlt";
      repo = "sddm-chili";
      rev = "6516d50176c3b34df29003726ef9708813d06271";
      fetchSubmodules = false;
      sha256 = "sha256-wxWsdRGC59YzDcSopDRzxg8TfjjmA3LHrdWjepTuzgw=";
    });
  };
  segue-ui-linux = {
    pname = "segue-ui-linux";
    version = "02f6e1e9290581e7fc9bf3beefc0ebc9ec8c1611";
    src = fetchFromGitHub ({
      owner = "mrbvrz";
      repo = "segoe-ui-linux";
      rev = "02f6e1e9290581e7fc9bf3beefc0ebc9ec8c1611";
      fetchSubmodules = false;
      sha256 = "sha256-zmR87N8loBtmi8qVH2kC0XSgydn8Sw570awYi/XAcuM=";
    });
  };
  sf-font-compact = {
    pname = "sf-font-compact";
    version = "0.0.0.20210625";
    src = fetchurl {
      url = "https://devimages-cdn.apple.com/design/resources/download/SF-Compact.dmg";
      sha256 = "sha256-SIht9sqmeijEeU4uLwm+tlZtFlTnD/G5GH8haUL6dlU=";
    };
  };
  sf-font-pro = {
    pname = "sf-font-pro";
    version = "0.0.0.20210625";
    src = fetchurl {
      url = "https://devimages-cdn.apple.com/design/resources/download/SF-Pro.dmg";
      sha256 = "sha256-g/SzgU3vOzm8uRuA6AN/N8Tnrl2Vpya58hx99dGfecI=";
    };
  };
  sf-mono = {
    pname = "sf-mono";
    version = "0.0.0.20210625";
    src = fetchurl {
      url = "https://devimages-cdn.apple.com/design/resources/download/SF-Mono.dmg";
      sha256 = "sha256-jnhTTmSy5J8MJotbsI8g5hxotgjvyDbccymjABwajYw=";
    };
  };
  snui = {
    pname = "snui";
    version = "34acc7f40bb96e69a9090f4d36a0a29d5e82d09a";
    src = fetchgit {
      url = "https://gitlab.com/snakedye/snui";
      rev = "34acc7f40bb96e69a9090f4d36a0a29d5e82d09a";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sha256 = "sha256-VJVnbIFRHolvpuAoXngrvOQfnly0UNB/t1MYddpoZMA=";
    };
  };
  spicetify-cli = {
    pname = "spicetify-cli";
    version = "1bda2cffa39bb71d8bad6ff58c9712d1d60aef0a";
    src = fetchFromGitHub ({
      owner = "spicetify";
      repo = "spicetify-cli";
      rev = "1bda2cffa39bb71d8bad6ff58c9712d1d60aef0a";
      fetchSubmodules = false;
      sha256 = "sha256-YgT4HlVqm58CEGoc/bNjo0xEzoNIcTpKGZkSdhcAgis=";
    });
  };
  spicetify-marketplace = {
    pname = "spicetify-marketplace";
    version = "51b72d680e2f76ac43e097f201622d3eaeabf2f9";
    src = fetchFromGitHub ({
      owner = "spicetify";
      repo = "spicetify-marketplace";
      rev = "51b72d680e2f76ac43e097f201622d3eaeabf2f9";
      fetchSubmodules = false;
      sha256 = "sha256-rqI0d2/wTMPTHCwr/jQu4YXfH7HTYF161atCvkVYKeo=";
    });
  };
  spicetify-themes = {
    pname = "spicetify-themes";
    version = "0f7a687e76c525e4013a7f82ccb15e11cc30475a";
    src = fetchFromGitHub ({
      owner = "spicetify";
      repo = "spicetify-themes";
      rev = "0f7a687e76c525e4013a7f82ccb15e11cc30475a";
      fetchSubmodules = false;
      sha256 = "sha256-OI0CiZE9GKaXVa5I8kCXlrfjX8izynKi5sJxPQK0Zd8=";
    });
  };
  spotify = {
    pname = "spotify";
    version = "1.1.84.716.gc5f8b819";
    src = fetchurl {
      url = "https://api.snapcraft.io/api/v1/snaps/download/pOBIoZ2LrCB3rDohMxoYGnbN14EHOgD7_60.snap";
      sha256 = "sha256-ho3WjGNjmHsERzC3FLgliuVSQWK2F5ec0ddQSfq6oyI=";
    };
  };
  spotify-1_1_83_954_gd226dfe8 = {
    pname = "spotify-1_1_83_954_gd226dfe8";
    version = "1.1.83.954.gd226dfe8";
    src = fetchurl {
      url = "https://api.snapcraft.io/api/v1/snaps/download/pOBIoZ2LrCB3rDohMxoYGnbN14EHOgD7_59.snap";
      sha256 = "sha256-IAktrSHXcoO6MxId4mJphMknOik0NG67Imwy4pLFCk8=";
    };
  };
  spotifyd = {
    pname = "spotifyd";
    version = "7146f64a819590a16de9991de74f8bc2fea9c2ac";
    src = fetchFromGitHub ({
      owner = "Spotifyd";
      repo = "spotifyd";
      rev = "7146f64a819590a16de9991de74f8bc2fea9c2ac";
      fetchSubmodules = false;
      sha256 = "sha256-6/mFCApGYiK6rYrzfxHuQI3a49bzkmpPcJw5gHCnUuY=";
    });
    cargoLock."Cargo.lock" = {
      lockFile = ./spotifyd-7146f64a819590a16de9991de74f8bc2fea9c2ac/Cargo.lock;
      outputHashes = {
        
      };
    };
  };
  steamcompmgr = {
    pname = "steamcompmgr";
    version = "f49432f14b296f3a84a265e4f138252875798a8d";
    src = fetchFromGitHub ({
      owner = "ChimeraOS";
      repo = "steamos-compositor-plus";
      rev = "f49432f14b296f3a84a265e4f138252875798a8d";
      fetchSubmodules = false;
      sha256 = "sha256-0X3M/GkWSHsmNMo65uxbnOEM9rzUZeukYEQBPXElUEU=";
    });
  };
  supergfxctl = {
    pname = "supergfxctl";
    version = "46f808da96802b6cc27e1b195bb500889fcf3a49";
    src = fetchgit {
      url = "https://gitlab.com/asus-linux/supergfxctl";
      rev = "46f808da96802b6cc27e1b195bb500889fcf3a49";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sha256 = "sha256-FTDJpmWsgF9CGfnVLYu7UosluQa/I5/omhOil8oEW64=";
    };
    cargoLock."Cargo.lock" = {
      lockFile = ./supergfxctl-46f808da96802b6cc27e1b195bb500889fcf3a49/Cargo.lock;
      outputHashes = {
        
      };
    };
  };
  sway-borders = {
    pname = "sway-borders";
    version = "3890d2a2fb255276993891e2da5f6cb33670efcd";
    src = fetchFromGitHub ({
      owner = "fluix-dev";
      repo = "sway-borders";
      rev = "3890d2a2fb255276993891e2da5f6cb33670efcd";
      fetchSubmodules = false;
      sha256 = "sha256-esMruivwq6XPPYJWaW825+d8rD8NOxbJQqVfAeBtXe0=";
    });
  };
  swaylock-effects = {
    pname = "swaylock-effects";
    version = "4b54b85d964243eef8d77ea0505eed728c62387f";
    src = fetchFromGitHub ({
      owner = "jirutka";
      repo = "swaylock-effects";
      rev = "4b54b85d964243eef8d77ea0505eed728c62387f";
      fetchSubmodules = false;
      sha256 = "sha256-QvMnJ5/uL/DvBhP9kagZJh9vwXBF2sCw3HWkfI4FDH0=";
    });
  };
  swhkd = {
    pname = "swhkd";
    version = "022466ec0bcca881d5034a2b23aab934cfd4578a";
    src = fetchFromGitHub ({
      owner = "waycrate";
      repo = "swhkd";
      rev = "022466ec0bcca881d5034a2b23aab934cfd4578a";
      fetchSubmodules = false;
      sha256 = "sha256-p4OU8tyd4pGr5PstuTu22wPzGvmftLBSsU7VB2Ix3Mk=";
    });
    cargoLock."Cargo.lock" = {
      lockFile = ./swhkd-022466ec0bcca881d5034a2b23aab934cfd4578a/Cargo.lock;
      outputHashes = {
        
      };
    };
  };
  tailscale = {
    pname = "tailscale";
    version = "4a82b317b793d34d7d3323287ea665278a93a413";
    src = fetchFromGitHub ({
      owner = "tailscale";
      repo = "tailscale";
      rev = "4a82b317b793d34d7d3323287ea665278a93a413";
      fetchSubmodules = false;
      sha256 = "sha256-GxNQZrQuSKGmCaUJtI4AeGef5AQh0IXxrrDavmzvp7o=";
    });
  };
  tailscale-systray = {
    pname = "tailscale-systray";
    version = "8e943debe7801a79c0e146cf9f53f0b9aa9d5e72";
    src = fetchFromGitHub ({
      owner = "mattn";
      repo = "tailscale-systray";
      rev = "8e943debe7801a79c0e146cf9f53f0b9aa9d5e72";
      fetchSubmodules = false;
      sha256 = "sha256-iIKth2qPc8S5h0ebasoiIGiEEy+mip8Ris1HWW/p6Ko=";
    });
  };
  teams = {
    pname = "teams";
    version = "1.5.00.10453";
    src = fetchurl {
      url = "https://packages.microsoft.com/repos/ms-teams/pool/main/t/teams/teams_1.5.00.10453_amd64.deb";
      sha256 = "sha256-fLVw2axSMetuaoRzjg+x4DRYY8WP5TQbL7LbfF6LFfA=";
    };
  };
  trackma = {
    pname = "trackma";
    version = "0a2558e34914a6c0802cad33e658c731c92d3b52";
    src = fetchurl {
      url = "https://github.com/z411/trackma/archive/master.tar.gz";
      sha256 = "sha256-x8D5XrvMjNV2R5CXs/ficOhwXNTRglsQxPhfvuUF4nM=";
    };
  };
  ulauncher = {
    pname = "ulauncher";
    version = "c9145fa20ca021685fef55d891cbed6ae70da47a";
    src = fetchFromGitHub ({
      owner = "Ulauncher";
      repo = "Ulauncher";
      rev = "c9145fa20ca021685fef55d891cbed6ae70da47a";
      fetchSubmodules = false;
      sha256 = "sha256-YCYFq+Dmj14tb6kov1O8LBRMdIv8LUdsDV3tMs8+IiY=";
    });
  };
  uniemoji = {
    pname = "uniemoji";
    version = "c9d758fd40b74ba043b1ef89a11b858fc08f65b1";
    src = fetchFromGitHub ({
      owner = "salty-horse";
      repo = "ibus-uniemoji";
      rev = "c9d758fd40b74ba043b1ef89a11b858fc08f65b1";
      fetchSubmodules = false;
      sha256 = "sha256-Yr4hr2GDJOKnYuorOfT99fYqtFvtO2MHnEJpFhZoRbU=";
    });
  };
  vaultwarden = {
    pname = "vaultwarden";
    version = "1.25.2";
    src = fetchFromGitHub ({
      owner = "dani-garcia";
      repo = "vaultwarden";
      rev = "1.25.2";
      fetchSubmodules = false;
      sha256 = "sha256-6CpdvLCw7SUmWm9NHAxFAo454Rrp1FloDp67YAr0pjQ=";
    });
    cargoLock."Cargo.lock" = {
      lockFile = ./vaultwarden-1.25.2/Cargo.lock;
      outputHashes = {
        
      };
    };
  };
  vaultwarden-vault = {
    pname = "vaultwarden-vault";
    version = "2022.9.0";
    src = fetchurl {
      url = "https://github.com/dani-garcia/bw_web_builds/releases/download/v2022.9.0/bw_web_v2022.9.0.tar.gz";
      sha256 = "sha256-RFwM8kYu58dg/6s3EJ1xsyaoVOlqTEe1epS5XQw4Qok=";
    };
  };
  ventoy = {
    pname = "ventoy";
    version = "1.0.79";
    src = fetchurl {
      url = "https://github.com/ventoy/Ventoy/releases/download/v1.0.79/ventoy-1.0.79-linux.tar.gz";
      sha256 = "sha256-azkoDquN/i01QHsd1wJG79kdhHhBvXzPXIRnjKHAHNE=";
    };
  };
  vimPlugins-yuck = {
    pname = "vimPlugins-yuck";
    version = "6dc3da77c53820c32648cf67cbdbdfb6994f4e08";
    src = fetchFromGitHub ({
      owner = "elkowar";
      repo = "yuck.vim";
      rev = "6dc3da77c53820c32648cf67cbdbdfb6994f4e08";
      fetchSubmodules = false;
      sha256 = "sha256-lp7qJWkvelVfoLCyI0aAiajTC+0W1BzDhmtta7tnICE=";
    });
  };
  waydroid = {
    pname = "waydroid";
    version = "68ed713ca10e9570ec0dbea72a3ba04a4ca54b4e";
    src = fetchFromGitHub ({
      owner = "waydroid";
      repo = "waydroid";
      rev = "68ed713ca10e9570ec0dbea72a3ba04a4ca54b4e";
      fetchSubmodules = false;
      sha256 = "sha256-YEqx5R6LXUYBGcUDMCdqOfb/fIaf0rSupSmOg/nhQCY=";
    });
  };
  wgcf = {
    pname = "wgcf";
    version = "140f9832314247e5a122ca252d139449255ad7c3";
    src = fetchFromGitHub ({
      owner = "ViRb3";
      repo = "wgcf";
      rev = "140f9832314247e5a122ca252d139449255ad7c3";
      fetchSubmodules = false;
      sha256 = "sha256-tUEeczs3J//2ZrZkAsSn3C+GScRPaIgheRM2wByS8p4=";
    });
  };
  whitesur-gtk-theme = {
    pname = "whitesur-gtk-theme";
    version = "950aed042e8bd4d11bdbc4cb53b4a721e5185e0f";
    src = fetchFromGitHub ({
      owner = "vinceliuice";
      repo = "whitesur-gtk-theme";
      rev = "950aed042e8bd4d11bdbc4cb53b4a721e5185e0f";
      fetchSubmodules = false;
      sha256 = "sha256-Iu2d8hAgWvrxGT5dsqBVCl4KPOep4AVnF4kRMczhD1A=";
    });
  };
  whitesur-icon-theme = {
    pname = "whitesur-icon-theme";
    version = "1fc9d502bd2941d04fec66c4e10372a7978b5c98";
    src = fetchFromGitHub ({
      owner = "vinceliuice";
      repo = "whitesur-icon-theme";
      rev = "1fc9d502bd2941d04fec66c4e10372a7978b5c98";
      fetchSubmodules = false;
      sha256 = "sha256-pcvRD4CUwUT46/kmMbnerj5mqPCcHIRreVIh9wz6Kfg=";
    });
  };
  widevine-cdm = {
    pname = "widevine-cdm";
    version = "4.10.2449.0";
    src = fetchurl {
      url = "https://dl.google.com/widevine-cdm/4.10.2449.0-linux-x64.zip";
      sha256 = "sha256-vsr6eymKqU2Y6HM4DPHnUfVkB2BQ1HSSWvk1tNhUA5Y=";
    };
  };
  wii-u-gc-adapter = {
    pname = "wii-u-gc-adapter";
    version = "fa098efa7f6b34f8cd82e2c249c81c629901976c";
    src = fetchFromGitHub ({
      owner = "ToadKing";
      repo = "wii-u-gc-adapter";
      rev = "fa098efa7f6b34f8cd82e2c249c81c629901976c";
      fetchSubmodules = false;
      sha256 = "sha256-wm0vDU7QckFvpgI50PG4/elgPEkfr8xTmroz8kE6QMo=";
    });
  };
  ydotool = {
    pname = "ydotool";
    version = "81195bfde8f9b5218e6e87bbd6f6e362a7653aad";
    src = fetchFromGitHub ({
      owner = "ReimuNotMoe";
      repo = "ydotool";
      rev = "81195bfde8f9b5218e6e87bbd6f6e362a7653aad";
      fetchSubmodules = false;
      sha256 = "sha256-maXXGCqB8dkGO8956hsKSwM4HQdYn6z1jBFENQ9sKcA=";
    });
  };
  yubikey-agent = {
    pname = "yubikey-agent";
    version = "6d9db9c29100daacbe83e74653c79c94acc5958d";
    src = fetchFromGitHub ({
      owner = "FiloSottile";
      repo = "yubikey-agent";
      rev = "6d9db9c29100daacbe83e74653c79c94acc5958d";
      fetchSubmodules = false;
      sha256 = "sha256-mrdZcKD61S94ZkVH93SJhozmsXW/22ho08WMCrDEwhs=";
    });
  };
  yubikey-guide = {
    pname = "yubikey-guide";
    version = "5eeae2be7e988871c8db8a54f4ae7f393c9317ae";
    src = fetchFromGitHub ({
      owner = "drduh";
      repo = "YubiKey-Guide";
      rev = "5eeae2be7e988871c8db8a54f4ae7f393c9317ae";
      fetchSubmodules = false;
      sha256 = "sha256-QuTBK2FpAXiF2IPoD4CeQ1/UZZ8KwAJ2H8Pl668iMfY=";
    });
  };
  zoom-us = {
    pname = "zoom-us";
    version = "5.11.10.4400";
    src = fetchurl {
      url = "https://cdn.zoom.us/prod/5.11.10.4400/zoom_x86_64.pkg.tar.xz";
      sha256 = "sha256-Pi1MtuCHzkQACamsNOIS6pbM03L1CmyosbpdrYVNCkQ=";
    };
  };
}
