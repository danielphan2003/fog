# This file was generated by nvfetcher, please do not modify it manually.
{ fetchgit, fetchurl, fetchFromGitHub, dockerTools }:
{
  adguardhome-darwin-amd64 = {
    pname = "adguardhome-darwin-amd64";
    version = "0.108.0-b.25";
    src = fetchurl {
      url = "https://github.com/AdguardTeam/AdGuardHome/releases/download/v0.108.0-b.25/AdGuardHome_darwin_amd64.zip";
      sha256 = "sha256-x6nH5Tli+kkGs0I1eGRP+EFURHzLYq65z6f433tcQCg=";
    };
  };
  adguardhome-linux-386 = {
    pname = "adguardhome-linux-386";
    version = "0.108.0-b.25";
    src = fetchurl {
      url = "https://github.com/AdguardTeam/AdGuardHome/releases/download/v0.108.0-b.25/AdGuardHome_linux_386.tar.gz";
      sha256 = "sha256-EHbzkgneiYkabymcdNDeF7QSnr3M7tw6NpBoSINL6SY=";
    };
  };
  adguardhome-linux-amd64 = {
    pname = "adguardhome-linux-amd64";
    version = "0.108.0-b.25";
    src = fetchurl {
      url = "https://github.com/AdguardTeam/AdGuardHome/releases/download/v0.108.0-b.25/AdGuardHome_linux_amd64.tar.gz";
      sha256 = "sha256-Mhvjk87m7NrOrGSkzsmyfjEgpOR2PsyD0OpyI8hQzmU=";
    };
  };
  adguardhome-linux-arm64 = {
    pname = "adguardhome-linux-arm64";
    version = "0.108.0-b.25";
    src = fetchurl {
      url = "https://github.com/AdguardTeam/AdGuardHome/releases/download/v0.108.0-b.25/AdGuardHome_linux_arm64.tar.gz";
      sha256 = "sha256-NC4p08lB3mGeTfUpi066wE3jvEFWx5EZvRMYR16b7hY=";
    };
  };
  adl = {
    pname = "adl";
    version = "07a78f6cda45e3b365e1ab1dda3fb7f52e598127";
    src = fetchFromGitHub ({
      owner = "RaitaroH";
      repo = "adl";
      rev = "07a78f6cda45e3b365e1ab1dda3fb7f52e598127";
      fetchSubmodules = false;
      sha256 = "sha256-Yw/k2jusuhBa/H6/ZIxRcN7jdulWw9J37gLQUGGm060=";
    });
    date = "2023-01-24";
  };
  age-plugin-yubikey = {
    pname = "age-plugin-yubikey";
    version = "f34c534e8479e4c50095a98f48941d079bd179a3";
    src = fetchFromGitHub ({
      owner = "str4d";
      repo = "age-plugin-yubikey";
      rev = "f34c534e8479e4c50095a98f48941d079bd179a3";
      fetchSubmodules = false;
      sha256 = "sha256-lV8Cw3W3MSThZIhtaUMId50hZswEIIxMUmUiVIUfYFE=";
    });
    cargoLock."Cargo.lock" = {
      lockFile = ./age-plugin-yubikey-f34c534e8479e4c50095a98f48941d079bd179a3/Cargo.lock;
      outputHashes = {
        "yubikey-0.7.0" = "sha256-H8qFvbsArqnJbFUww3X3aUg7cvH0dEJrPs3a+s6y8QE=";
      };
    };
    date = "2023-01-03";
  };
  anime-downloader = {
    pname = "anime-downloader";
    version = "24de83d4ef392e17f39710cc9054ff90e3602533";
    src = fetchFromGitHub ({
      owner = "anime-dl";
      repo = "anime-downloader";
      rev = "24de83d4ef392e17f39710cc9054ff90e3602533";
      fetchSubmodules = false;
      sha256 = "sha256-yS3lkmoqVNORG+H6x95mt68N8wh8X5/gji1fAl1RPqg=";
    });
    date = "2022-09-12";
  };
  any-nix-shell = {
    pname = "any-nix-shell";
    version = "c8536fa43f0e8b506e9dd1a91e2f01ceccca0251";
    src = fetchFromGitHub ({
      owner = "haslersn";
      repo = "any-nix-shell";
      rev = "c8536fa43f0e8b506e9dd1a91e2f01ceccca0251";
      fetchSubmodules = false;
      sha256 = "sha256-dQd1/RYH/j3R7MgMoz3EZ2Nxf3XXYCTKxBvi+cvyV2g=";
    });
    date = "2022-12-11";
  };
  arkenfox-userjs = {
    pname = "arkenfox-userjs";
    version = "b99dd27de828be13530ce2f48c9178d34f5f82ab";
    src = fetchFromGitHub ({
      owner = "arkenfox";
      repo = "user.js";
      rev = "b99dd27de828be13530ce2f48c9178d34f5f82ab";
      fetchSubmodules = false;
      sha256 = "sha256-NGp5BLOQmiXsUh9nrXP+PeVXyK1c8Ij5EnwtFXAkD9w=";
    });
    date = "2023-01-27";
  };
  asusctl = {
    pname = "asusctl";
    version = "5600c51ba0fafcb250af12de96e651ead0e0cef8";
    src = fetchgit {
      url = "https://gitlab.com/asus-linux/asusctl";
      rev = "5600c51ba0fafcb250af12de96e651ead0e0cef8";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sha256 = "sha256-zebXl23TFJ0olvfmPkt57u9PvPt5Hzc9ef46IxPcvrU=";
    };
    cargoLock."Cargo.lock" = {
      lockFile = ./asusctl-5600c51ba0fafcb250af12de96e651ead0e0cef8/Cargo.lock;
      outputHashes = {
        "supergfxctl-5.0.2" = "sha256-oyOi6DLMtShY0V81l96zii1ibYCueNgHle+FGHmNv+M=";
        "ecolor-0.20.0" = "sha256-tnjFkaCWmCPGw3huQN9VOAeiH+zk3Zk9xYoRKmg2WQg=";
        "notify-rust-4.6.0" = "sha256-jhCgisA9f6AI9e9JQUYRtEt47gQnDv5WsdRKFoKvHJs=";
      };
    };
    date = "2023-01-25";
  };
  avizo = {
    pname = "avizo";
    version = "9cd5319503584e2e7e0e1ec1fe35c5b8c3e7262e";
    src = fetchFromGitHub ({
      owner = "misterdanb";
      repo = "avizo";
      rev = "9cd5319503584e2e7e0e1ec1fe35c5b8c3e7262e";
      fetchSubmodules = false;
      sha256 = "sha256-fW4y6Ml63XS0uVF6dBDOL9uKkoHE3L3ylwiHs6immrk=";
    });
    date = "2023-01-07";
  };
  awesome = {
    pname = "awesome";
    version = "b54e50ad6cfdcd864a21970b31378f7c64adf3f4";
    src = fetchFromGitHub ({
      owner = "awesomeWM";
      repo = "awesome";
      rev = "b54e50ad6cfdcd864a21970b31378f7c64adf3f4";
      fetchSubmodules = false;
      sha256 = "sha256-yDXC1PT5r0V6bbyk/Y6oBxvHE74q96cGKlo3C3OUobE=";
    });
    date = "2023-01-16";
  };
  awestore = {
    pname = "awestore";
    version = "5509786a1d5bc538dcdde027bd07b35bc29b0df1";
    src = fetchFromGitHub ({
      owner = "K4rakara";
      repo = "awestore";
      rev = "5509786a1d5bc538dcdde027bd07b35bc29b0df1";
      fetchSubmodules = false;
      sha256 = "sha256-VuMmEf5bCifXKHerTyFOYW4w4fXA83HM02Oj4VmQOeo=";
    });
    date = "2021-04-09";
  };
  betterdiscord-installer = {
    pname = "betterdiscord-installer";
    version = "1.2.0";
    src = fetchurl {
      url = "https://github.com/BetterDiscord/Installer/releases/download/v1.2.0/BetterDiscord-Linux.AppImage";
      sha256 = "sha256-oXrx+bQgVf8Pf70M4saXR2Kb2QmSMIGKhBDrTsN/CKU=";
    };
  };
  betterdiscordctl = {
    pname = "betterdiscordctl";
    version = "49a26fe82d441fbb8e51b0511969929f05cb2057";
    src = fetchFromGitHub ({
      owner = "bb010g";
      repo = "betterdiscordctl";
      rev = "49a26fe82d441fbb8e51b0511969929f05cb2057";
      fetchSubmodules = false;
      sha256 = "sha256-aBJDQAKmsBGoFaurADBTDqL9L/4+bWBk2y26vQ0LFx4=";
    });
    date = "2022-02-21";
  };
  bling = {
    pname = "bling";
    version = "af90ae42a91d825b17e129659522ff1643ce4908";
    src = fetchFromGitHub ({
      owner = "BlingCorp";
      repo = "bling";
      rev = "af90ae42a91d825b17e129659522ff1643ce4908";
      fetchSubmodules = false;
      sha256 = "sha256-mGm4Ydq1K4y2TLgqdYsjsSprk0KHRjSnQtYjqZ+mh2w=";
    });
    date = "2023-01-23";
  };
  bottles = {
    pname = "bottles";
    version = "6f2d8635a39c11bc0557a5955bbd8a9f3a577974";
    src = fetchFromGitHub ({
      owner = "bottlesdevs";
      repo = "Bottles";
      rev = "6f2d8635a39c11bc0557a5955bbd8a9f3a577974";
      fetchSubmodules = false;
      sha256 = "sha256-z/0QK6YBFoVcbuXBHMoQQORjPQeANNIlvBnMI2BAToU=";
    });
    date = "2023-01-27";
  };
  caddy = {
    pname = "caddy";
    version = "v2.6.2";
    src = fetchFromGitHub ({
      owner = "caddyserver";
      repo = "caddy";
      rev = "v2.6.2";
      fetchSubmodules = false;
      sha256 = "sha256-Tbf6RB3106OEZGc/Wx7vk+I82Z8/Q3WqnID4f8uZ6z0=";
    });
  };
  caddy-dist = {
    pname = "caddy-dist";
    version = "v2.6.2";
    src = fetchFromGitHub ({
      owner = "caddyserver";
      repo = "dist";
      rev = "v2.6.2";
      fetchSubmodules = false;
      sha256 = "sha256-bctRc2klaV2JbB4KRWk5n/ACQ/7KlgaMKHae7SIQs+0=";
    });
  };
  caddy-git = {
    pname = "caddy-git";
    version = "d73660f7c338cf4d12ba82c07e14df7f53593ea5";
    src = fetchFromGitHub ({
      owner = "caddyserver";
      repo = "caddy";
      rev = "d73660f7c338cf4d12ba82c07e14df7f53593ea5";
      fetchSubmodules = false;
      sha256 = "sha256-eZ/rjTEAAHT6d6Tc2v/cS01YTZeiLyN0RhcbNulUAUo=";
    });
    date = "2023-01-27";
  };
  cinny = {
    pname = "cinny";
    version = "4ea14c853ee7b9c1a211a08e386fb323870e2ec2";
    src = fetchFromGitHub ({
      owner = "cinnyapp";
      repo = "cinny";
      rev = "4ea14c853ee7b9c1a211a08e386fb323870e2ec2";
      fetchSubmodules = false;
      sha256 = "sha256-nnPQBPV1Qkeh4eEza5P+30fh6gopFVHKY9Yr2tYGh8A=";
    });
    date = "2023-01-15";
  };
  cinny-bin = {
    pname = "cinny-bin";
    version = "2.2.3";
    src = fetchurl {
      url = "https://github.com/cinnyapp/cinny/releases/download/v2.2.3/cinny-v2.2.3.tar.gz";
      sha256 = "sha256-Q6f24LRYCxdgAguUVl7jf7srkd2L1IptiBgHJQq2dHE=";
    };
  };
  cinny-desktop = {
    pname = "cinny-desktop";
    version = "5a2206fa1b12dc4b5baf181c09b2a23b46f3704d";
    src = fetchFromGitHub ({
      owner = "cinnyapp";
      repo = "cinny-desktop";
      rev = "5a2206fa1b12dc4b5baf181c09b2a23b46f3704d";
      fetchSubmodules = true;
      sha256 = "sha256-6hyHjJJU5TP7m1PZ2YF/RwgZcZwDfP9lJqYTUcWMQqM=";
    });
    cargoLock."src-tauri/Cargo.lock" = {
      lockFile = ./cinny-desktop-5a2206fa1b12dc4b5baf181c09b2a23b46f3704d/src-tauri/Cargo.lock;
      outputHashes = {
        
      };
    };
    date = "2023-01-15";
  };
  cinny-desktop-bin = {
    pname = "cinny-desktop-bin";
    version = "2.2.3";
    src = fetchurl {
      url = "https://github.com/cinnyapp/cinny-desktop/releases/download/v2.2.3/Cinny_desktop-x86_64.AppImage";
      sha256 = "sha256-xIRKTXGqsqip46adc3v8vWQoYgcDPw801cF10/UwGSs=";
    };
  };
  conduit = {
    pname = "conduit";
    version = "06df04f61c86c856cb7308b012a2d27695585862";
    src = fetchgit {
      url = "https://gitlab.com/famedly/conduit";
      rev = "06df04f61c86c856cb7308b012a2d27695585862";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sha256 = "sha256-pgXAb3ZFh0cPVLED2DkuDxGxMEPRlnMWl5gJG8MOG2c=";
    };
    cargoLock."Cargo.lock" = {
      lockFile = ./conduit-06df04f61c86c856cb7308b012a2d27695585862/Cargo.lock;
      outputHashes = {
        "ruma-0.7.4" = "sha256-ztobLdOXSGyK1YcPMMIycO3ZmnjxG5mLkHltf0Fbs8s=";
        "reqwest-0.11.9" = "sha256-wH/q7REnkz30ENBIK5Rlxnc1F6vOyuEANMHFmiVPaGw=";
        "heed-0.10.6" = "sha256-rm02pJ6wGYN4SsAbp85jBVHDQ5ITjZZd+79EC2ubRsY=";
      };
    };
    date = "2023-01-27";
  };
  conduit-toolbox = {
    pname = "conduit-toolbox";
    version = "805baa070528e6db7161ef35b75180e70d896541";
    src = fetchFromGitHub ({
      owner = "ShadowJonathan";
      repo = "conduit_toolbox";
      rev = "805baa070528e6db7161ef35b75180e70d896541";
      fetchSubmodules = false;
      sha256 = "sha256-vQ4r2Cvf2okeJ5qGIQ8mHoUmmrbUbY+bIEpee7XH6zA=";
    });
    cargoLock."Cargo.lock" = {
      lockFile = ./conduit-toolbox-805baa070528e6db7161ef35b75180e70d896541/Cargo.lock;
      outputHashes = {
        "heed-0.10.6" = "sha256-rm02pJ6wGYN4SsAbp85jBVHDQ5ITjZZd+79EC2ubRsY=";
      };
    };
    date = "2022-03-19";
  };
  cups-pdf = {
    pname = "cups-pdf";
    version = "3.0.1";
    src = fetchurl {
      url = "https://www.cups-pdf.de/src/cups-pdf_3.0.1.tar.gz";
      sha256 = "sha256-c4Zp7f9/FGn+XkESAth/k7oltF8zKmI/tgfUnFmqlTE=";
    };
  };
  discord-canary = {
    pname = "discord-canary";
    version = "0.0.146";
    src = fetchurl {
      url = "https://dl-canary.discordapp.net/apps/linux/0.0.146/discord-canary-0.0.146.tar.gz";
      sha256 = "sha256-5GiG+RGqwHUdIeSVWpbaIw3sTuZbvIQeetXSEvSqTls=";
    };
  };
  dnscrypt-proxy2 = {
    pname = "dnscrypt-proxy2";
    version = "5438eed2f43e0ac26cc3c441f680faecbaf4762d";
    src = fetchFromGitHub ({
      owner = "DNSCrypt";
      repo = "dnscrypt-proxy";
      rev = "5438eed2f43e0ac26cc3c441f680faecbaf4762d";
      fetchSubmodules = false;
      sha256 = "sha256-8GCU3a5qjCNEI8gcmzNesgvZEiyfiNfxewy6Yiy5oi8=";
    });
    date = "2023-01-08";
  };
  doggo = {
    pname = "doggo";
    version = "8492ccdc61f097da3a8e5dcb2080e77650af8d2d";
    src = fetchFromGitHub ({
      owner = "mr-karan";
      repo = "doggo";
      rev = "8492ccdc61f097da3a8e5dcb2080e77650af8d2d";
      fetchSubmodules = false;
      sha256 = "sha256-qc6RYz2bVaY/IBGIXUYO6wyh7iUDAJ1ASCK0dFwZo6s=";
    });
    date = "2023-01-03";
  };
  dribbblish-dynamic-theme = {
    pname = "dribbblish-dynamic-theme";
    version = "8da36b8e5f07a20af2ca8a1dd3c81eedf7282fa1";
    src = fetchFromGitHub ({
      owner = "JulienMaille";
      repo = "dribbblish-dynamic-theme";
      rev = "8da36b8e5f07a20af2ca8a1dd3c81eedf7282fa1";
      fetchSubmodules = false;
      sha256 = "sha256-70wtFxJOY9h4moT4hCTs4yfp2wGM39i5qnJerBVkpV4=";
    });
    date = "2022-05-18";
  };
  droidcam = {
    pname = "droidcam";
    version = "323fafd60a8ed0039d94e7995afae68cf9643993";
    src = fetchFromGitHub ({
      owner = "aramg";
      repo = "droidcam";
      rev = "323fafd60a8ed0039d94e7995afae68cf9643993";
      fetchSubmodules = false;
      sha256 = "sha256-NgKhWlK7MSU1JRxvECiRkt+iYcIVftTIbw5kJP6u/AE=";
    });
    date = "2023-01-21";
  };
  duf = {
    pname = "duf";
    version = "318f3b7848fd64a567fa798b3538f2de4a9dcfe4";
    src = fetchFromGitHub ({
      owner = "muesli";
      repo = "duf";
      rev = "318f3b7848fd64a567fa798b3538f2de4a9dcfe4";
      fetchSubmodules = false;
      sha256 = "sha256-DsJpuntHGT/7eRrJTgIOttqtJy2ZSwgWZ2higldIGok=";
    });
    date = "2022-12-03";
  };
  eww = {
    pname = "eww";
    version = "6a64a286291456984011c4fb160def106ee55dff";
    src = fetchFromGitHub ({
      owner = "elkowar";
      repo = "eww";
      rev = "6a64a286291456984011c4fb160def106ee55dff";
      fetchSubmodules = false;
      sha256 = "sha256-IliWerC9qk4MxYNRoe/8uuQZkqjPJPsph46fmVBvo9U=";
    });
    cargoLock."Cargo.lock" = {
      lockFile = ./eww-6a64a286291456984011c4fb160def106ee55dff/Cargo.lock;
      outputHashes = {
        
      };
    };
    date = "2023-01-05";
  };
  fake-background-webcam = {
    pname = "fake-background-webcam";
    version = "efda82945820db1400106a52a209e14b8c33d9d9";
    src = fetchFromGitHub ({
      owner = "fangfufu";
      repo = "Linux-Fake-Background-Webcam";
      rev = "efda82945820db1400106a52a209e14b8c33d9d9";
      fetchSubmodules = false;
      sha256 = "sha256-6i7QsErGCFo5jxeXceDVijp6B6S4tmNYhE560j0T7GE=";
    });
    date = "2022-09-25";
  };
  firefox-sidebar = {
    pname = "firefox-sidebar";
    version = "d99c43774b56226834c273c131563dfa9625f58d";
    src = fetchFromGitHub ({
      owner = "drannex42";
      repo = "FirefoxSidebar";
      rev = "d99c43774b56226834c273c131563dfa9625f58d";
      fetchSubmodules = false;
      sha256 = "sha256-LMqmawJbPc8trFYhxRHG1i24R62CZa5wj9+J6yejhCY=";
    });
    date = "2022-12-01";
  };
  flameshot = {
    pname = "flameshot";
    version = "05c3b8746aedbc296e0c6271c131f8e2e0183b1a";
    src = fetchFromGitHub ({
      owner = "flameshot-org";
      repo = "flameshot";
      rev = "05c3b8746aedbc296e0c6271c131f8e2e0183b1a";
      fetchSubmodules = false;
      sha256 = "sha256-t2YQvxEEc80UyOjUmsttrEwOrZHUlIL2fR7dT00yQFg=";
    });
    date = "2023-01-26";
  };
  fluffychat = {
    pname = "fluffychat";
    version = "215e78f3d5624e12e63d7af4602368b434ad7bb2";
    src = fetchgit {
      url = "https://gitlab.com/famedly/fluffychat";
      rev = "215e78f3d5624e12e63d7af4602368b434ad7bb2";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sha256 = "sha256-SMKT+bHO3zt+5fi1YQaHPnZgEGk1Bme7Jf4+jl4iCtU=";
    };
    date = "2023-01-27";
  };
  flyingfox = {
    pname = "flyingfox";
    version = "4a6ba46cafc189ad628ee13bb8d3eef3e69fbced";
    src = fetchFromGitHub ({
      owner = "akshat46";
      repo = "FlyingFox";
      rev = "4a6ba46cafc189ad628ee13bb8d3eef3e69fbced";
      fetchSubmodules = false;
      sha256 = "sha256-ZUTQ2VvIAEPbLnEX2AYScAGTM3hWp7vUuGCz2wd8qG0=";
    });
    date = "2021-10-16";
  };
  frece = {
    pname = "frece";
    version = "d4071c6c852e68dda82b7ab0bfb0f141b39d4484";
    src = fetchFromGitHub ({
      owner = "YodaEmbedding";
      repo = "frece";
      rev = "d4071c6c852e68dda82b7ab0bfb0f141b39d4484";
      fetchSubmodules = false;
      sha256 = "sha256-lLSRLcnUJqWCEIJEOVHANzxTI+xR3VegRHtjHHI0gTs=";
    });
    cargoLock."Cargo.lock" = {
      lockFile = ./frece-d4071c6c852e68dda82b7ab0bfb0f141b39d4484/Cargo.lock;
      outputHashes = {
        
      };
    };
    date = "2021-10-09";
  };
  fx_cast = {
    pname = "fx_cast";
    version = "1e4575790fef9b8af220908aec5aa327071d8623";
    src = fetchFromGitHub ({
      owner = "hensm";
      repo = "fx_cast";
      rev = "1e4575790fef9b8af220908aec5aa327071d8623";
      fetchSubmodules = false;
      sha256 = "sha256-/+Tqkb8ERWZlVsBmEsjQNIO0uOFRkJStqD9qTBxjwE8=";
    });
    date = "2023-01-07";
  };
  godu = {
    pname = "godu";
    version = "fcc4ddea7ba6d20d944931d4c5c07af1757d7422";
    src = fetchFromGitHub ({
      owner = "viktomas";
      repo = "godu";
      rev = "fcc4ddea7ba6d20d944931d4c5c07af1757d7422";
      fetchSubmodules = false;
      sha256 = "sha256-fJeSUAuNELZZ1DcybNsYd2ZX93VYWsLum5tHp68ZVlo=";
    });
    date = "2022-11-20";
  };
  guiscrcpy = {
    pname = "guiscrcpy";
    version = "332175e50b63d1503c08afc5d839fc3f3fc50aba";
    src = fetchFromGitHub ({
      owner = "srevinsaju";
      repo = "guiscrcpy";
      rev = "332175e50b63d1503c08afc5d839fc3f3fc50aba";
      fetchSubmodules = false;
      sha256 = "sha256-E8+uGQ4I/6wxhqoGY575J4tWWmBK/eorkmBBeia4TD4=";
    });
    date = "2023-01-02";
  };
  hyprland = {
    pname = "hyprland";
    version = "af37a3895f810ebe3cc58600b0d9b2fcf449b2d1";
    src = fetchFromGitHub ({
      owner = "hyprwm";
      repo = "hyprland";
      rev = "af37a3895f810ebe3cc58600b0d9b2fcf449b2d1";
      fetchSubmodules = false;
      sha256 = "sha256-/lOvQnwIxDjz8E0VKKlkHqZ/DjBYXy7miEDegiGMY8M=";
    });
    date = "2023-01-27";
  };
  ibus = {
    pname = "ibus";
    version = "2a235c8c33f1de56df4873fe3662d5beb1355715";
    src = fetchFromGitHub ({
      owner = "ibus";
      repo = "ibus";
      rev = "2a235c8c33f1de56df4873fe3662d5beb1355715";
      fetchSubmodules = false;
      sha256 = "sha256-3cMMZqjLB72Yxp03VPxARZx3B7yzkmLVkqFbkcpAyFs=";
    });
    date = "2023-01-25";
  };
  ibus-bamboo = {
    pname = "ibus-bamboo";
    version = "d14fd96d860866f8a813d2203829eb10c8a880c0";
    src = fetchFromGitHub ({
      owner = "BambooEngine";
      repo = "ibus-bamboo";
      rev = "d14fd96d860866f8a813d2203829eb10c8a880c0";
      fetchSubmodules = false;
      sha256 = "sha256-ECJU/alzzDMWU01z3PssmztSG9SyIeWjqpvGEomh8Pw=";
    });
    date = "2022-10-31";
  };
  inih = {
    pname = "inih";
    version = "r56";
    src = fetchFromGitHub ({
      owner = "benhoyt";
      repo = "inih";
      rev = "r56";
      fetchSubmodules = false;
      sha256 = "sha256-7k3i3pElihastUDrdf9DyRZMe2UNFckfLUFGb4rbWLo=";
    });
  };
  jibri = {
    pname = "jibri";
    version = "8.0-141-gf226708";
    src = fetchurl {
      url = "https://download.jitsi.org/unstable/jibri_8.0-141-gf226708-1_all.deb";
      sha256 = "sha256-oHoPxznW6+t1e21tVzxNeIor7CQvSxG/QVEpJLiLQ0Q=";
    };
  };
  jicofo = {
    pname = "jicofo";
    version = "1.0-987";
    src = fetchurl {
      url = "https://download.jitsi.org/unstable/jicofo_1.0-987-1_all.deb";
      sha256 = "sha256-VK4Ck+OU6xv/Lma4YpXduPThej2wopbs+OkBC2SOkJU=";
    };
  };
  jitsi-meet = {
    pname = "jitsi-meet";
    version = "1.0.6946";
    src = fetchurl {
      url = "https://download.jitsi.org/jitsi-meet/src/jitsi-meet-1.0.6946.tar.bz2";
      sha256 = "sha256-SsjqOmk92ptnYyMALPF5z1a1ba7h11Fvv7JwVbeAKeg=";
    };
  };
  jitsi-meet-prosody = {
    pname = "jitsi-meet-prosody";
    version = "1.0.6946";
    src = fetchurl {
      url = "https://download.jitsi.org/unstable/jitsi-meet-prosody_1.0.6946-1_all.deb";
      sha256 = "sha256-NkuCAF5JSLAefk6mVCqSF07z2NTrq7jhtIAiJk4uQLM=";
    };
  };
  jitsi-videobridge2 = {
    pname = "jitsi-videobridge2";
    version = "2.2-70-g0aa0b1f6";
    src = fetchurl {
      url = "https://download.jitsi.org/unstable/jitsi-videobridge2_2.2-70-g0aa0b1f6-1_all.deb";
      sha256 = "sha256-albvyMvD5+DrYAZWQ2idNfxhuGD2faqHfuQU6v4Q+MQ=";
    };
  };
  layout-machi = {
    pname = "layout-machi";
    version = "497d076903e6f5375c0456593f24adcca3db07e6";
    src = fetchFromGitHub ({
      owner = "xinhaoyuan";
      repo = "layout-machi";
      rev = "497d076903e6f5375c0456593f24adcca3db07e6";
      fetchSubmodules = false;
      sha256 = "sha256-6bBtXEich+b+un+KGva+nL7AWx0fSsEgma5IMusbqNA=";
    });
    date = "2022-11-07";
  };
  leonflix = {
    pname = "leonflix";
    version = "0.7.0";
    src = fetchurl {
      url = "https://leonflix.net/downloads/Leonflix.AppImage";
      sha256 = "sha256-Fok8OPP9dWjFyIVWUud+UDLiG/lUBNisuCcI0ipjHi4=";
    };
  };
  light = {
    pname = "light";
    version = "8e162c5a6359d13d2bca5e5eb71f5280b4708593";
    src = fetchFromGitHub ({
      owner = "haikarainen";
      repo = "light";
      rev = "8e162c5a6359d13d2bca5e5eb71f5280b4708593";
      fetchSubmodules = false;
      sha256 = "sha256-NrsAfQqtG0tWofDINb6HqskMcZ1JWzqByKeVFFarfIU=";
    });
    date = "2022-10-16";
  };
  lua-pam = {
    pname = "lua-pam";
    version = "482071137257e55dac62a510f792104a9d910ea1";
    src = fetchFromGitHub ({
      owner = "RMTT";
      repo = "lua-pam";
      rev = "482071137257e55dac62a510f792104a9d910ea1";
      fetchSubmodules = false;
      sha256 = "sha256-Kn0ozckmgxs19TmCuSChdiYMch92eif7DTEK+UoBtjw=";
    });
    date = "2020-07-29";
  };
  manix = {
    pname = "manix";
    version = "d08e7ca185445b929f097f8bfb1243a8ef3e10e4";
    src = fetchFromGitHub ({
      owner = "mlvzk";
      repo = "manix";
      rev = "d08e7ca185445b929f097f8bfb1243a8ef3e10e4";
      fetchSubmodules = false;
      sha256 = "sha256-GqPuYscLhkR5E2HnSFV4R48hCWvtM3C++3zlJhiK/aw=";
    });
    date = "2021-04-20";
  };
  mapcidr = {
    pname = "mapcidr";
    version = "v1.0.3";
    src = fetchFromGitHub ({
      owner = "projectdiscovery";
      repo = "mapcidr";
      rev = "v1.0.3";
      fetchSubmodules = false;
      sha256 = "sha256-dsHTnaK1Bna6Gbr/J+PYjeZ0WqJh696sliTd5JF1C+o=";
    });
  };
  matrix-appservice-discord = {
    pname = "matrix-appservice-discord";
    version = "ece29f4f5f613b259e7920f445d288b787ceb1d2";
    src = fetchFromGitHub ({
      owner = "matrix-org";
      repo = "matrix-appservice-discord";
      rev = "ece29f4f5f613b259e7920f445d288b787ceb1d2";
      fetchSubmodules = false;
      sha256 = "sha256-g681w7RD96/xKP+WnIyY4bcVHVQhysgDPZo4TgCRiuY=";
    });
    date = "2022-11-10";
  };
  microsoft-edge-beta = {
    pname = "microsoft-edge-beta";
    version = "110.0.1587.22";
    src = fetchurl {
      url = "https://packages.microsoft.com/repos/edge/pool/main/m/microsoft-edge-beta/microsoft-edge-beta_110.0.1587.22-1_amd64.deb";
      sha256 = "sha256-S7pLDbmFvmkEZ8ZMvfBEmI8J5UMJghH3hPE56E2UHTI=";
    };
  };
  microsoft-edge-dev = {
    pname = "microsoft-edge-dev";
    version = "111.0.1633.0";
    src = fetchurl {
      url = "https://packages.microsoft.com/repos/edge/pool/main/m/microsoft-edge-dev/microsoft-edge-dev_111.0.1633.0-1_amd64.deb";
      sha256 = "sha256-R9wjwIKEc+ZwbRnAnBMftKpFUzrtdc2oLITRahFrI6A=";
    };
  };
  microsoft-edge-stable = {
    pname = "microsoft-edge-stable";
    version = "109.0.1518.70";
    src = fetchurl {
      url = "https://packages.microsoft.com/repos/edge/pool/main/m/microsoft-edge-stable/microsoft-edge-stable_109.0.1518.70-1_amd64.deb";
      sha256 = "sha256-PCoFb+neyIq7KyQWLynGrkqpxFFd+iz/ER1LpYcpBY4=";
    };
  };
  minecraft-wayland = {
    pname = "minecraft-wayland";
    version = "370ce5b95e3ae9bc4618fb45113bc641fbb13867";
    src = fetchFromGitHub ({
      owner = "Admicos";
      repo = "minecraft-wayland";
      rev = "370ce5b95e3ae9bc4618fb45113bc641fbb13867";
      fetchSubmodules = false;
      sha256 = "sha256-RPRg6Gd7N8yyb305V607NTC1kUzvyKiWsh6QlfHW+JE=";
    });
    date = "2022-08-22";
  };
  netdata = {
    pname = "netdata";
    version = "1.37.1";
    src = fetchurl {
      url = "https://github.com/netdata/netdata/archive/refs/tags/v1.37.1.tar.gz";
      sha256 = "sha256-7nHleiQ+01ARbERqAtb/KCo8g2whRuTFA/++CHPMt+A=";
    };
  };
  netdata-go-d-plugin = {
    pname = "netdata-go-d-plugin";
    version = "0.49.2";
    src = fetchurl {
      url = "https://github.com/netdata/go.d.plugin/archive/refs/tags/v0.49.2.tar.gz";
      sha256 = "sha256-57RKgch8MKKTT+71UCtasn5ZE8G765keTxnK1fvNprI=";
    };
  };
  nix-zsh-completions = {
    pname = "nix-zsh-completions";
    version = "468d8cf752a62b877eba1a196fbbebb4ce4ebb6f";
    src = fetchFromGitHub ({
      owner = "spwhitt";
      repo = "nix-zsh-completions";
      rev = "468d8cf752a62b877eba1a196fbbebb4ce4ebb6f";
      fetchSubmodules = false;
      sha256 = "sha256-TWgo56l+FBXssOYWlAfJ5j4pOHNmontOEolcGdihIJs=";
    });
    date = "2021-02-22";
  };
  ntfs2btrfs = {
    pname = "ntfs2btrfs";
    version = "2b4ac4f68323c9231b03b80fa1ffad5d7a08bd35";
    src = fetchFromGitHub ({
      owner = "maharmstone";
      repo = "ntfs2btrfs";
      rev = "2b4ac4f68323c9231b03b80fa1ffad5d7a08bd35";
      fetchSubmodules = false;
      sha256 = "sha256-eTxG5ic10F8YS8ZAn2jynC3HAdJcZ47zlLwpIraT9e0=";
    });
    date = "2022-12-22";
  };
  nwg-drawer = {
    pname = "nwg-drawer";
    version = "820848d9845592da785fae466742091d8e42767b";
    src = fetchFromGitHub ({
      owner = "nwg-piotr";
      repo = "nwg-drawer";
      rev = "820848d9845592da785fae466742091d8e42767b";
      fetchSubmodules = false;
      sha256 = "sha256-34C0JmsPuDqR3QGmGf14naGOu9xPtPbpdWUvkbilkqs=";
    });
    date = "2023-01-22";
  };
  ny-font = {
    pname = "ny-font";
    version = "0.0.0.20221012";
    src = fetchurl {
      url = "https://devimages-cdn.apple.com/design/resources/download/NY.dmg";
      sha256 = "sha256-HuAgyTh+Z1K+aIvkj5VvL6QqfmpMj6oLGGXziAM5C+A=";
    };
  };
  ouch = {
    pname = "ouch";
    version = "ba7d1189da64efce683fccf6b8b2e54af91503da";
    src = fetchFromGitHub ({
      owner = "ouch-org";
      repo = "ouch";
      rev = "ba7d1189da64efce683fccf6b8b2e54af91503da";
      fetchSubmodules = false;
      sha256 = "sha256-V/JpEVIPU1UQnKvx18ja7CtqP+NOLQ0Wcl0GU5kua+A=";
    });
    cargoLock."Cargo.lock" = {
      lockFile = ./ouch-ba7d1189da64efce683fccf6b8b2e54af91503da/Cargo.lock;
      outputHashes = {
        
      };
    };
    date = "2023-01-10";
  };
  paper = {
    pname = "paper";
    version = "35ecb75c6776615f1db5dfb861fbe407263d8057";
    src = fetchgit {
      url = "https://gitlab.com/snakedye/paper";
      rev = "35ecb75c6776615f1db5dfb861fbe407263d8057";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sha256 = "sha256-D0VMUZ7QN4xTcTBulzSlt7IvjRxPYFiTs1AcuhNKrek=";
    };
    cargoLock."Cargo.lock" = {
      lockFile = ./paper-35ecb75c6776615f1db5dfb861fbe407263d8057/Cargo.lock;
      outputHashes = {
        "snui-0.1.0" = "sha256-y61EWKr5qD9OFrMVQs9SVyLCqq5lOwLXCAo2w0AYt7M=";
      };
    };
    date = "2021-09-22";
  };
  picom = {
    pname = "picom";
    version = "364463feaf89fa0b7ad82f15af6576400b6daec4";
    src = fetchFromGitHub ({
      owner = "yshui";
      repo = "picom";
      rev = "364463feaf89fa0b7ad82f15af6576400b6daec4";
      fetchSubmodules = false;
      sha256 = "sha256-ukYfi86muqe05IEeteaWTdINT7Bq+PTvt64Jq+j1ed0=";
    });
    date = "2023-01-17";
  };
  playit-agent = {
    pname = "playit-agent";
    version = "185bfb0a75d3af75d915c84fa1bcd514653220f1";
    src = fetchFromGitHub ({
      owner = "playit-cloud";
      repo = "playit-agent";
      rev = "185bfb0a75d3af75d915c84fa1bcd514653220f1";
      fetchSubmodules = false;
      sha256 = "sha256-7dC7aq79DCWudXt4YwCSPY6Xs6M62i6TJq3gBacKpHc=";
    });
    cargoLock."Cargo.lock" = {
      lockFile = ./playit-agent-185bfb0a75d3af75d915c84fa1bcd514653220f1/Cargo.lock;
      outputHashes = {
        
      };
    };
    date = "2022-11-02";
  };
  plymouth-themes = {
    pname = "plymouth-themes";
    version = "bf2f570bee8e84c5c20caac353cbe1d811a4745f";
    src = fetchFromGitHub ({
      owner = "adi1090x";
      repo = "plymouth-themes";
      rev = "bf2f570bee8e84c5c20caac353cbe1d811a4745f";
      fetchSubmodules = false;
      sha256 = "sha256-VNGvA8ujwjpC2rTVZKrXni2GjfiZk7AgAn4ZB4Baj2k=";
    });
    date = "2021-07-12";
  };
  pmbootstrap = {
    pname = "pmbootstrap";
    version = "1.50.1";
    src = fetchurl {
      url = "https://pypi.io/packages/source/p/pmbootstrap/pmbootstrap-1.50.1.tar.gz";
      sha256 = "sha256-2S3I3J3wmRkVSUshyQCUTuYgHLsDMnXZQHt7KySBzIY=";
    };
  };
  pmbootstrap-test = {
    pname = "pmbootstrap-test";
    version = "9975d373b09e13fff84c316c92f211c00985c14c";
    src = fetchgit {
      url = "https://gitlab.com/postmarketOS/pmbootstrap";
      rev = "9975d373b09e13fff84c316c92f211c00985c14c";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sha256 = "sha256-g+zGpEFE5q3l/qzM6zJ5FQ+IVt50+rSu8Vh3IB9MICE=";
    };
    date = "2023-01-22";
  };
  pure = {
    pname = "pure";
    version = "2f13dea466466dde1ba844ba5211e7556f4ae2db";
    src = fetchFromGitHub ({
      owner = "sindresorhus";
      repo = "pure";
      rev = "2f13dea466466dde1ba844ba5211e7556f4ae2db";
      fetchSubmodules = false;
      sha256 = "sha256-YfasTKCABvMtncrfoWR1Su9QxzCqPED18/BTXaJHttg=";
    });
    date = "2023-01-06";
  };
  pythonPackages-cfscrape = {
    pname = "pythonPackages-cfscrape";
    version = "e510962c608382bcef5de75033d60cc98cb9561d";
    src = fetchurl {
      url = "https://github.com/Anorov/cloudflare-scrape/archive/master.tar.gz";
      sha256 = "sha256-+pFN1Ts23Ggm6YZueyLdQaMJCfox/CRmMelzGvWRtEw=";
    };
    date = "2020-03-23";
  };
  pythonPackages-cmapy = {
    pname = "pythonPackages-cmapy";
    version = "0.6.6";
    src = fetchurl {
      url = "https://pypi.io/packages/source/c/cmapy/cmapy-0.6.6.tar.gz";
      sha256 = "sha256-y1KmswV8SaFG+wlkuDAvL7fWHf5q5t4amLY2qs6AUlU=";
    };
  };
  pythonPackages-mediapipe = {
    pname = "pythonPackages-mediapipe";
    version = "0.8.8";
    src = fetchurl {
      url = "https://files.pythonhosted.org/packages/ed/5f/188b509ab93e740f3fd8d9179de0ef9340852b48f1f5d671cd090a54996f/mediapipe-0.8.8-cp39-cp39-manylinux_2_17_x86_64.manylinux2014_x86_64.whl";
      sha256 = "sha256-qW1D06zrH8HnZiQoO5PC+8mK8z1HcZJxU9hmwyi8FZc=";
    };
  };
  pythonPackages-pySmartDL = {
    pname = "pythonPackages-pySmartDL";
    version = "b93df794e1e60017c42d9520ac097b6fd38c2e8b";
    src = fetchurl {
      url = "https://github.com/iTaybb/pySmartDL/archive/master.tar.gz";
      sha256 = "sha256-26TGmz9eIXm6epKUnvNDQymRBOC3TcpxCXFbhuVfVto=";
    };
    date = "2020-09-19";
  };
  pywalfox = {
    pname = "pywalfox";
    version = "2.7.4";
    src = fetchurl {
      url = "https://pypi.io/packages/source/p/pywalfox/pywalfox-2.7.4.tar.gz";
      sha256 = "sha256-Wec9fic4lXT7gBY04D2EcfCb/gYoZcrYA/aMRWaA7WY=";
    };
  };
  quibble = {
    pname = "quibble";
    version = "3efc4f8670fdf9bdb82b58391b107699b537458f";
    src = fetchFromGitHub ({
      owner = "maharmstone";
      repo = "quibble";
      rev = "3efc4f8670fdf9bdb82b58391b107699b537458f";
      fetchSubmodules = false;
      sha256 = "sha256-IGfVLeOA7puojDchfOsFDz+r1Y8j5kJkvScVZ2EdENI=";
    });
    date = "2023-01-25";
  };
  rainfox = {
    pname = "rainfox";
    version = "231a8434a3c1aedd64dd8ca4cf2e67318440c9ff";
    src = fetchFromGitHub ({
      owner = "1280px";
      repo = "rainfox";
      rev = "231a8434a3c1aedd64dd8ca4cf2e67318440c9ff";
      fetchSubmodules = false;
      sha256 = "sha256-muDz1rFTvWOZGrEzql+VZbs42bS09BREjKS5OpBu2g8=";
    });
    date = "2022-02-04";
  };
  redshift = {
    pname = "redshift";
    version = "7da875d34854a6a34612d5ce4bd8718c32bec804";
    src = fetchFromGitHub ({
      owner = "minus7";
      repo = "redshift";
      rev = "7da875d34854a6a34612d5ce4bd8718c32bec804";
      fetchSubmodules = false;
      sha256 = "sha256-pyR7xNdi/83NSoC2WIrX8U+A6nU+vZBlePLXrQZnc1k=";
    });
    date = "2019-08-24";
  };
  retroarch = {
    pname = "retroarch";
    version = "482c8ccf3573c39220b9b5415be046bc3cb16839";
    src = fetchFromGitHub ({
      owner = "libretro";
      repo = "retroarch";
      rev = "482c8ccf3573c39220b9b5415be046bc3cb16839";
      fetchSubmodules = false;
      sha256 = "sha256-q/uikRapHU6dqd/RbKrOV3VpLQRD0ULAdyk/smxhZjA=";
    });
    date = "2023-01-28";
  };
  rofi-wayland = {
    pname = "rofi-wayland";
    version = "5d4a3e14788945a9a1c7d18993151a3b00649373";
    src = fetchFromGitHub ({
      owner = "lbonn";
      repo = "rofi";
      rev = "5d4a3e14788945a9a1c7d18993151a3b00649373";
      fetchSubmodules = true;
      sha256 = "sha256-mtvFfUOMWSlMtFuYCY5bCOcEHmflwScVk3/NDEYzDSo=";
    });
    date = "2022-12-27";
  };
  rustdesk = {
    pname = "rustdesk";
    version = "2e848061cb30c67873ff1187cc0b28e28a1ba7ce";
    src = fetchFromGitHub ({
      owner = "rustdesk";
      repo = "rustdesk";
      rev = "2e848061cb30c67873ff1187cc0b28e28a1ba7ce";
      fetchSubmodules = false;
      sha256 = "sha256-TpagTSIka2dh6XGr3ifUd9X4G9/tx0NQ48T1RRxNUSs=";
    });
    cargoLock."Cargo.lock" = {
      lockFile = ./rustdesk-2e848061cb30c67873ff1187cc0b28e28a1ba7ce/Cargo.lock;
      outputHashes = {
        "default-net-0.11.0" = "sha256-Hjc5Abd/D8k/SQjjyVr0u+Gf1WpxjIc2r7tmcHshm7M=";
        "trayicon-0.1.3-1" = "sha256-shLsOncL3ySRN/m9SjvjurneMaXhGDWks0QF3pVz6J0=";
        "tfc-0.6.1" = "sha256-Laah+4iRf0pN13TZaNJKXj8ty28o75mD5VNhVn05el4=";
        "rust-pulsectl-0.2.12" = "sha256-8jXTspWvjONFcvw9/Z8C43g4BuGZ3rsG32tvLMQbtbM=";
        "confy-0.4.0" = "sha256-e91cvEixhpPzIthAxzTa3fDY6eCsHUy/eZQAqs7QTDo=";
        "x11-2.19.0" = "sha256-GDCeKzUtvaLeBDmPQdyr499EjEfT6y4diBMzZVEptzc=";
        "parity-tokio-ipc-0.7.3-1" = "sha256-eULJePtBu0iBI3It/bPH0h82Obsb1PJALgwYwrnCFYI=";
        "impersonate_system-0.1.0" = "sha256-qbaTw9gxMKDjX5pKdUrKlmIxCxWwb99YuWPDvD2A3kY=";
        "tokio-socks-0.5.1-1" = "sha256-45QQ6FrhGU9uEhbKXTKd/mY6MDumO6p46NmlakdyDQk=";
        "hwcodec-0.1.0" = "sha256-d/u20YHGteK6ONHgIL+xrJ8UVUvwiK/DZ1F3MzRc8SU=";
        "mouce-0.2.1" = "sha256-3PtNEmVMXgqKV4r3KiKTkk4oyCt4BKynniJREE+RyFk=";
        "rdev-0.5.0-2" = "sha256-duALF6cQgwA0vYS7/GsF8ei0W8gmIBEo30EeWa3bhu4=";
        "sciter-rs-0.5.57" = "sha256-ZZnZDhMjK0LjgmK0da1yvB0uoKueLhhhQtzmjoN+1R0=";
        "magnum-opus-0.4.0" = "sha256-NWYobxN2X18f0l8GmLB9dULm+GZD04mI03EvRy05K10=";
        "evdev-0.11.5" = "sha256-aoPmjGi/PftnH6ClEWXHvIj0X3oh15ZC1q7wPC1XPr0=";
      };
    };
    date = "2023-01-28";
  };
  rustdesk-server = {
    pname = "rustdesk-server";
    version = "2830be95a75e8cdd59c65697d2e77762982e305b";
    src = fetchFromGitHub ({
      owner = "rustdesk";
      repo = "rustdesk-server";
      rev = "2830be95a75e8cdd59c65697d2e77762982e305b";
      fetchSubmodules = false;
      sha256 = "sha256-QSHj5gk3XH4Ixwlh9h2fJYoph4HawJPMewQbd8WLKQA=";
    });
    cargoLock."Cargo.lock" = {
      lockFile = ./rustdesk-server-2830be95a75e8cdd59c65697d2e77762982e305b/Cargo.lock;
      outputHashes = {
        "confy-0.4.0" = "sha256-e91cvEixhpPzIthAxzTa3fDY6eCsHUy/eZQAqs7QTDo=";
        "async-speed-limit-0.3.1" = "sha256-iOel6XA07RPrBjQAFLnxXX4VBpDrYZaqQc9clnsOorI=";
        "tokio-socks-0.5.1" = "sha256-inmAJk0fAlsVNIwfD/M+htwIdQHwGSTRrEy6N/mspMI=";
      };
    };
    date = "2023-01-27";
  };
  sddm = {
    pname = "sddm";
    version = "3e486499b9300ce8f9c62bd102e5119b27a2fad1";
    src = fetchFromGitHub ({
      owner = "sddm";
      repo = "sddm";
      rev = "3e486499b9300ce8f9c62bd102e5119b27a2fad1";
      fetchSubmodules = false;
      sha256 = "sha256-Y9WPm0MLWl/s0e0aoKKk0SSojqBrI/RdyxkgOz2Tk38=";
    });
    date = "2022-11-23";
  };
  sddm-chili = {
    pname = "sddm-chili";
    version = "6516d50176c3b34df29003726ef9708813d06271";
    src = fetchFromGitHub ({
      owner = "MarianArlt";
      repo = "sddm-chili";
      rev = "6516d50176c3b34df29003726ef9708813d06271";
      fetchSubmodules = false;
      sha256 = "sha256-wxWsdRGC59YzDcSopDRzxg8TfjjmA3LHrdWjepTuzgw=";
    });
    date = "2018-08-26";
  };
  segue-ui-linux = {
    pname = "segue-ui-linux";
    version = "02f6e1e9290581e7fc9bf3beefc0ebc9ec8c1611";
    src = fetchFromGitHub ({
      owner = "mrbvrz";
      repo = "segoe-ui-linux";
      rev = "02f6e1e9290581e7fc9bf3beefc0ebc9ec8c1611";
      fetchSubmodules = false;
      sha256 = "sha256-zmR87N8loBtmi8qVH2kC0XSgydn8Sw570awYi/XAcuM=";
    });
    date = "2020-12-04";
  };
  sf-font-compact = {
    pname = "sf-font-compact";
    version = "0.0.0.20221012";
    src = fetchurl {
      url = "https://devimages-cdn.apple.com/design/resources/download/SF-Compact.dmg";
      sha256 = "sha256-0mUcd7H7SxZN3J1I+T4SQrCsJjHL0GuDCjjZRi9KWBM=";
    };
  };
  sf-font-pro = {
    pname = "sf-font-pro";
    version = "0.0.0.20221012";
    src = fetchurl {
      url = "https://devimages-cdn.apple.com/design/resources/download/SF-Pro.dmg";
      sha256 = "sha256-g/eQoYqTzZwrXvQYnGzDFBEpKAPC8wHlUw3NlrBabHw=";
    };
  };
  sf-mono = {
    pname = "sf-mono";
    version = "0.0.0.20221012";
    src = fetchurl {
      url = "https://devimages-cdn.apple.com/design/resources/download/SF-Mono.dmg";
      sha256 = "sha256-q69tYs1bF64YN6tAo1OGczo/YDz2QahM9Zsdf7TKrDk=";
    };
  };
  snui = {
    pname = "snui";
    version = "c1e321bee3a294e248d6630af9057a12bca34cf7";
    src = fetchgit {
      url = "https://gitlab.com/snakedye/snui";
      rev = "c1e321bee3a294e248d6630af9057a12bca34cf7";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sha256 = "sha256-9IoA4i2JTkwuyinYFgBnqzJS0b92OqtFjXSUmKHiSzc=";
    };
    date = "2023-01-06";
  };
  spicetify-cli = {
    pname = "spicetify-cli";
    version = "5fed7a25663b292718555ff4e6cd20713392ac46";
    src = fetchFromGitHub ({
      owner = "spicetify";
      repo = "spicetify-cli";
      rev = "5fed7a25663b292718555ff4e6cd20713392ac46";
      fetchSubmodules = false;
      sha256 = "sha256-NerZQPP+eqSSOpCyd1ELnizpUZK4CsXM5naS2MvWvqI=";
    });
    date = "2023-01-27";
  };
  spicetify-marketplace = {
    pname = "spicetify-marketplace";
    version = "2c8216205c8a9973628bf9e413a55e1ec856d34c";
    src = fetchFromGitHub ({
      owner = "spicetify";
      repo = "spicetify-marketplace";
      rev = "2c8216205c8a9973628bf9e413a55e1ec856d34c";
      fetchSubmodules = false;
      sha256 = "sha256-K9qZHwyPfy04r2cQJMwwa+0gXI+h6WFqKuX/lPeDMao=";
    });
    date = "2023-01-23";
  };
  spicetify-themes = {
    pname = "spicetify-themes";
    version = "f8c91b681feb3389b3c12c6a70424aba84b3afd2";
    src = fetchFromGitHub ({
      owner = "spicetify";
      repo = "spicetify-themes";
      rev = "f8c91b681feb3389b3c12c6a70424aba84b3afd2";
      fetchSubmodules = false;
      sha256 = "sha256-ZuwaQhJ6dCuhw0bW5kB3vxH3GfDPM7y3l//5F6xRyV0=";
    });
    date = "2023-01-23";
  };
  spotify = {
    pname = "spotify";
    version = "1.1.99.878.g1e4ccc6e";
    src = fetchurl {
      url = "https://api.snapcraft.io/api/v1/snaps/download/pOBIoZ2LrCB3rDohMxoYGnbN14EHOgD7_62.snap";
      sha256 = "sha256-a0aGti+OK2vRzNCexJAvg/jpwxHRtAfysjLKMvEZTlk=";
    };
  };
  spotify-1_1_83_954_gd226dfe8 = {
    pname = "spotify-1_1_83_954_gd226dfe8";
    version = "1.1.83.954.gd226dfe8";
    src = fetchurl {
      url = "https://api.snapcraft.io/api/v1/snaps/download/pOBIoZ2LrCB3rDohMxoYGnbN14EHOgD7_59.snap";
      sha256 = "sha256-IAktrSHXcoO6MxId4mJphMknOik0NG67Imwy4pLFCk8=";
    };
  };
  spotifyd = {
    pname = "spotifyd";
    version = "946d81c6fc219efd6b4509167f71aa90b1cbd858";
    src = fetchFromGitHub ({
      owner = "Spotifyd";
      repo = "spotifyd";
      rev = "946d81c6fc219efd6b4509167f71aa90b1cbd858";
      fetchSubmodules = false;
      sha256 = "sha256-EYsvOcXpfUqbEpexUlpT5ZYIUkB138E1mMlxfmmUEcI=";
    });
    cargoLock."Cargo.lock" = {
      lockFile = ./spotifyd-946d81c6fc219efd6b4509167f71aa90b1cbd858/Cargo.lock;
      outputHashes = {
        
      };
    };
    date = "2023-01-08";
  };
  steamcompmgr = {
    pname = "steamcompmgr";
    version = "e4b99dd5f56a388aa24fe3055d0e983cb3d5d32a";
    src = fetchFromGitHub ({
      owner = "ChimeraOS";
      repo = "steamos-compositor-plus";
      rev = "e4b99dd5f56a388aa24fe3055d0e983cb3d5d32a";
      fetchSubmodules = false;
      sha256 = "sha256-Jbdo4RJHfQPqoiTRtR5B6II/LkrefagIky2hUYCcDGc=";
    });
    date = "2022-12-17";
  };
  supergfxctl = {
    pname = "supergfxctl";
    version = "e59c026eb6c5d4ee32c3b576b6b63fc07f436ef6";
    src = fetchgit {
      url = "https://gitlab.com/asus-linux/supergfxctl";
      rev = "e59c026eb6c5d4ee32c3b576b6b63fc07f436ef6";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sha256 = "sha256-oyOi6DLMtShY0V81l96zii1ibYCueNgHle+FGHmNv+M=";
    };
    cargoLock."Cargo.lock" = {
      lockFile = ./supergfxctl-e59c026eb6c5d4ee32c3b576b6b63fc07f436ef6/Cargo.lock;
      outputHashes = {
        
      };
    };
    date = "2023-01-06";
  };
  sway-borders = {
    pname = "sway-borders";
    version = "c3ab09150bb9d64d43b8733ff0ea3a6d116e1398";
    src = fetchFromGitHub ({
      owner = "fluix-dev";
      repo = "sway-borders";
      rev = "c3ab09150bb9d64d43b8733ff0ea3a6d116e1398";
      fetchSubmodules = false;
      sha256 = "sha256-ShWbbwuXQ9pTTuPhXof+v+tKXs7FLs+zMLKpLDYrjKE=";
    });
    date = "2022-11-21";
  };
  swaylock-effects = {
    pname = "swaylock-effects";
    version = "a7691b86dabe5241c7292c7b8d0551d579ba1848";
    src = fetchFromGitHub ({
      owner = "jirutka";
      repo = "swaylock-effects";
      rev = "a7691b86dabe5241c7292c7b8d0551d579ba1848";
      fetchSubmodules = false;
      sha256 = "sha256-MKmWVYssO9HAcP5uqwpy9kDa6/kfZyV2NI7ibozt7Ug=";
    });
    date = "2023-01-19";
  };
  swhkd = {
    pname = "swhkd";
    version = "3b19fc33b32efde88311579152a1078a8004397c";
    src = fetchFromGitHub ({
      owner = "waycrate";
      repo = "swhkd";
      rev = "3b19fc33b32efde88311579152a1078a8004397c";
      fetchSubmodules = false;
      sha256 = "sha256-245Y3UicW33hrQ6Mtf07I9vsWSpuijIEoEhxIKtjVQE=";
    });
    cargoLock."Cargo.lock" = {
      lockFile = ./swhkd-3b19fc33b32efde88311579152a1078a8004397c/Cargo.lock;
      outputHashes = {
        
      };
    };
    date = "2022-12-23";
  };
  tailscale = {
    pname = "tailscale";
    version = "e51cf1b09d0de6319c776c456e8b23be426a1b74";
    src = fetchFromGitHub ({
      owner = "tailscale";
      repo = "tailscale";
      rev = "e51cf1b09d0de6319c776c456e8b23be426a1b74";
      fetchSubmodules = false;
      sha256 = "sha256-6fjiNmyYuXsLurug02XtdA4DDNUa2IsICjliHMrYDLk=";
    });
    date = "2023-01-27";
  };
  tailscale-systray = {
    pname = "tailscale-systray";
    version = "e7f8893684e7b8779f34045ca90e5abe6df6056d";
    src = fetchFromGitHub ({
      owner = "mattn";
      repo = "tailscale-systray";
      rev = "e7f8893684e7b8779f34045ca90e5abe6df6056d";
      fetchSubmodules = false;
      sha256 = "sha256-3kozp6jq0xGllxoK2lGCNUahy/FvXyq11vNSxfDehKE=";
    });
    date = "2022-10-20";
  };
  teams = {
    pname = "teams";
    version = "1.5.00.23861";
    src = fetchurl {
      url = "https://packages.microsoft.com/repos/ms-teams/pool/main/t/teams/teams_1.5.00.23861_amd64.deb";
      sha256 = "sha256-h0YnCeJX//l4TegJVZtavV3HrxjYUF2Fa5KmaYmZW8E=";
    };
  };
  trackma = {
    pname = "trackma";
    version = "cac51d8d7bae2b173ebede6570922b01281236d9";
    src = fetchurl {
      url = "https://github.com/z411/trackma/archive/master.tar.gz";
      sha256 = "sha256-Hm+YS3dMICoRT/oZbWV1gDu2+HkNx5LWMtjX0I99iwk=";
    };
    date = "2023-01-16";
  };
  ulauncher = {
    pname = "ulauncher";
    version = "af6bef385d5868edabe542ce7becd1e418202748";
    src = fetchFromGitHub ({
      owner = "Ulauncher";
      repo = "Ulauncher";
      rev = "af6bef385d5868edabe542ce7becd1e418202748";
      fetchSubmodules = false;
      sha256 = "sha256-aHTKeFoSoOGBkdTHFm8d9KaQMkflmTz+2kx1sAQMea0=";
    });
    date = "2023-01-24";
  };
  uniemoji = {
    pname = "uniemoji";
    version = "7972b04a0fb9caeb180244011c8b7ebbb72bff60";
    src = fetchFromGitHub ({
      owner = "salty-horse";
      repo = "ibus-uniemoji";
      rev = "7972b04a0fb9caeb180244011c8b7ebbb72bff60";
      fetchSubmodules = false;
      sha256 = "sha256-MX1kGj0eXfUEtDEOFVrK+wHZUhUY2KiHhHrNZwAocLE=";
    });
    date = "2022-11-04";
  };
  vaultwarden = {
    pname = "vaultwarden";
    version = "1.27.0";
    src = fetchFromGitHub ({
      owner = "dani-garcia";
      repo = "vaultwarden";
      rev = "1.27.0";
      fetchSubmodules = false;
      sha256 = "sha256-QvU1Y3syr6PZbTRebbZF4sEzI4lIj1enJe2F/gGfvQM=";
    });
    cargoLock."Cargo.lock" = {
      lockFile = ./vaultwarden-1.27.0/Cargo.lock;
      outputHashes = {
        "multer-2.0.4" = "sha256-962onbcPUjkoIGSQTnF8m1fkamwThpPj4uBJ8EqbouQ=";
      };
    };
  };
  vaultwarden-vault = {
    pname = "vaultwarden-vault";
    version = "2023.1.1";
    src = fetchurl {
      url = "https://github.com/dani-garcia/bw_web_builds/releases/download/v2023.1.1/bw_web_v2023.1.1.tar.gz";
      sha256 = "sha256-P6ujENub+BjMPb90gXM1dWd1PlTg0RTSIW3/dXTNx1E=";
    };
  };
  ventoy = {
    pname = "ventoy";
    version = "1.0.88";
    src = fetchurl {
      url = "https://github.com/ventoy/Ventoy/releases/download/v1.0.88/ventoy-1.0.88-linux.tar.gz";
      sha256 = "sha256-mg1dzXREIhO9LsoCEauuBR9ESGHM3RvoFN+5vHU0HDA=";
    };
  };
  vimPlugins-yuck = {
    pname = "vimPlugins-yuck";
    version = "9b5e0370f70cc30383e1dabd6c215475915fe5c3";
    src = fetchFromGitHub ({
      owner = "elkowar";
      repo = "yuck.vim";
      rev = "9b5e0370f70cc30383e1dabd6c215475915fe5c3";
      fetchSubmodules = false;
      sha256 = "sha256-F7aS8d6jJQQoIGkG2o4KNnDZAIrq0c+otIHvjdoGbtY=";
    });
    date = "2022-10-29";
  };
  waydroid = {
    pname = "waydroid";
    version = "574405d056f08aae90769c7515d8cccf78b1fec9";
    src = fetchFromGitHub ({
      owner = "waydroid";
      repo = "waydroid";
      rev = "574405d056f08aae90769c7515d8cccf78b1fec9";
      fetchSubmodules = false;
      sha256 = "sha256-Icj9NdL4NIS39pA7czxeY3MrraXGXNXyJ4jzrh3IEz0=";
    });
    date = "2023-01-27";
  };
  wgcf = {
    pname = "wgcf";
    version = "140f9832314247e5a122ca252d139449255ad7c3";
    src = fetchFromGitHub ({
      owner = "ViRb3";
      repo = "wgcf";
      rev = "140f9832314247e5a122ca252d139449255ad7c3";
      fetchSubmodules = false;
      sha256 = "sha256-tUEeczs3J//2ZrZkAsSn3C+GScRPaIgheRM2wByS8p4=";
    });
    date = "2022-06-03";
  };
  whitesur-gtk-theme = {
    pname = "whitesur-gtk-theme";
    version = "c64c88eec5aabb3487538b78342a170c4bb33f90";
    src = fetchFromGitHub ({
      owner = "vinceliuice";
      repo = "whitesur-gtk-theme";
      rev = "c64c88eec5aabb3487538b78342a170c4bb33f90";
      fetchSubmodules = false;
      sha256 = "sha256-+arq4wquPiG9ZOcKruI1ApuxjHJVcO/mlx9RWqL3UFY=";
    });
    date = "2023-01-23";
  };
  whitesur-icon-theme = {
    pname = "whitesur-icon-theme";
    version = "380f753e3665a6d3bc80b61e88ac75733eafe457";
    src = fetchFromGitHub ({
      owner = "vinceliuice";
      repo = "whitesur-icon-theme";
      rev = "380f753e3665a6d3bc80b61e88ac75733eafe457";
      fetchSubmodules = false;
      sha256 = "sha256-ib/SH9tdbcG88N4MSZJwckNxpN5+q4KTyieqF/NVyJw=";
    });
    date = "2023-01-14";
  };
  widevine-cdm = {
    pname = "widevine-cdm";
    version = "4.10.2557.0";
    src = fetchurl {
      url = "https://dl.google.com/widevine-cdm/4.10.2557.0-linux-x64.zip";
      sha256 = "sha256-29C8D5UbUbBNF8thyjFlbu6Xm5XXcXyqhvNUKpOlQrI=";
    };
  };
  wii-u-gc-adapter = {
    pname = "wii-u-gc-adapter";
    version = "fa098efa7f6b34f8cd82e2c249c81c629901976c";
    src = fetchFromGitHub ({
      owner = "ToadKing";
      repo = "wii-u-gc-adapter";
      rev = "fa098efa7f6b34f8cd82e2c249c81c629901976c";
      fetchSubmodules = false;
      sha256 = "sha256-wm0vDU7QckFvpgI50PG4/elgPEkfr8xTmroz8kE6QMo=";
    });
    date = "2021-09-06";
  };
  ydotool = {
    pname = "ydotool";
    version = "724799af1cf64758f35cf677e5d27cd0881d03db";
    src = fetchFromGitHub ({
      owner = "ReimuNotMoe";
      repo = "ydotool";
      rev = "724799af1cf64758f35cf677e5d27cd0881d03db";
      fetchSubmodules = false;
      sha256 = "sha256-sluGcbVgMZCyOroZ5qvrUIKhVh2m9v0sbd3pGpUaynI=";
    });
    date = "2023-01-15";
  };
  yubikey-agent = {
    pname = "yubikey-agent";
    version = "2e5376c5ec006250c12c1b6de65fa91de9afe687";
    src = fetchFromGitHub ({
      owner = "FiloSottile";
      repo = "yubikey-agent";
      rev = "2e5376c5ec006250c12c1b6de65fa91de9afe687";
      fetchSubmodules = false;
      sha256 = "sha256-lEeISFOnihe89WLOwO24GqnhSA1DdidV9GmUYsa+lmQ=";
    });
    date = "2022-12-31";
  };
  yubikey-guide = {
    pname = "yubikey-guide";
    version = "6437be8fda70926f11afa22a61287af63d943c24";
    src = fetchFromGitHub ({
      owner = "drduh";
      repo = "YubiKey-Guide";
      rev = "6437be8fda70926f11afa22a61287af63d943c24";
      fetchSubmodules = false;
      sha256 = "sha256-jm0WRnyRmwylAqRVyK0mUzYxEkmk3g0u6f+DIdMZW3I=";
    });
    date = "2022-12-26";
  };
  zoom-us = {
    pname = "zoom-us";
    version = "5.13.5.431";
    src = fetchurl {
      url = "https://cdn.zoom.us/prod/5.13.5.431/zoom_x86_64.pkg.tar.xz";
      sha256 = "sha256-R0IMV/+R7AGFy/ZvNyyvIBv10t1x1U1X6jdHoo6UHKY=";
    };
  };
}
